...
/// leave the Switch Account menu unchanged. Show both accounts with three percentage cards side by
/// side, including Fable, colored by proximity to the limit and red at 100%. Omit "used" and
/// "limit reached" captions. Keep numbered steps with one animated line beneath the active step,
/// replacing the rejected spinner around the number. The automatic third step is "Continue
/// Session"; manual switches wait for the user's next message. No heading spinner, repeated status
