...
//!
//! Port of `packages/core-ui/chat/session-chat-slash-commands.ts`. The catalogs are a curated
//! snapshot of what each agent CLI intercepts, not a runtime discovery, and they also feed the
//! send classification, so a wrong name costs more than a missing one: the TypeScript file is the
//! place that documents how to re-verify them against the binary the user runs, and both copies
//! change together.
//!
...
const CLAUDE_CODE: &[SlashCommand] = &[
    SlashCommand {
        name: "add-dir",
