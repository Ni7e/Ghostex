...
Migration copies existing raw values into IndexedDB with a strict commit, reads them back, and removes a legacy source only when the copied value and current source match. A bounded receipt per legacy namespace prevents concurrent startup from replaying data that has already been imported and acknowledged. Existing protected data may exceed a new budget; migration still preserves it, and subsequent shrinking writes remain possible. Legacy outbox and conversation databases are migrated through the same adapter. Unknown or conflicting sources remain available for inspection.

Storage usage lives on Settings > Debugging, above About. The page starts with Show debug UI controls. The inspector and Ghostex folder statistics are hidden and lazy-loaded only while that switch is enabled and the Debugging page is open. It reports committed bytes separately from pending writes, feature ownership, budgets, entry counts, the largest keys, write rate, failures, and unknown entries. Activity and pending-save metadata are aggregated across open pages, so Settings also sees failures in a composer or another view. Retry reaches every page; cache clearing also discards failed pending writes for that cache. Its clear action accepts only disposable caches.
