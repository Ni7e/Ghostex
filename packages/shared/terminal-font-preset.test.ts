...
describe('normalizeTerminalFontPreset', () => {
  ...
  test('should map resolved font stacks back to their presets', () => {
    ...
    expect(normalizeTerminalFontPreset(CROSS_PLATFORM_MONO_TERMINAL_FONT_FAMILY)).toBe('Cross Platform Mono');
    expect(normalizeTerminalFontPreset(DROID_SANS_MONO_TERMINAL_FONT_FAMILY)).toBe('Droid Sans Mono (Linux Default)');
...
describe('terminal font preset helpers', () => {
  ...
  test('should detect the preset from the resolved font family', () => {
    ...
    expect(getTerminalFontPresetFromFontFamily(JETBRAINS_MONO_TERMINAL_FONT_FAMILY)).toBe('JetBrains Mono');
  });
