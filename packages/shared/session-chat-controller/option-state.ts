...
export interface SessionChatOptionPersistence {
  read: (key: string | undefined) => SessionChatOptionState;
  write: (key: string | undefined, state: SessionChatOptionState) => void;
}
...
export function createSessionChatOptionState(
  ...
  persistence: SessionChatOptionPersistence = {
    read: readStoredSessionChatOptions,
    write: writeStoredSessionChatOptions,
  }
) {
  let state: SessionChatOptionState = catalog ? seedSessionChatOptionState(catalog, persistence.read(storageKey)) : {};
  ...
  const publish = (next: SessionChatOptionState): void => {
    ...
    state = next;
    persistence.write(storageKey, next);
