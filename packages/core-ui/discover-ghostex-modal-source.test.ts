...
const discoverModalSource = readFileSync(new URL('./discover-ghostex-modal.tsx', import.meta.url), 'utf8');
...
function readPngSize(buffer: Buffer): { height: number; width: number } {
  expect([...buffer.subarray(0, pngSignature.length)], 'asset should be PNG data').toEqual(pngSignature);
  return {
...
describe('discover ghostex modal source', () => {
  ...
  test('keeps featured README screenshot assets visible', () => {
    ...
    */
    for (const imagePath of featureScreenshotPaths) {
      ...
      expect(imageData.byteLength, `${imagePath} should contain real screenshot detail`).toBeGreaterThan(100_000);
      expect(width, `${imagePath} should keep full screenshot width`).toBeGreaterThanOrEqual(2000);
      expect(height, `${imagePath} should keep full screenshot height`).toBeGreaterThanOrEqual(1200);
    }
  });
  ...
  test('keeps feature copy above a full-width contained image', () => {
    ...
    */
    ...
    expect(featureDescriptionStyles).toContain('color: color-mix(in srgb, var(--foreground) 70%, #8b8f98 30%);');
