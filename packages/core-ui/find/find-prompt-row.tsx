  ...
  return (
    ...
    >
      <span className='flex w-3 shrink-0 justify-center pt-1'>
        {row.favorite ? <IconStarFilled aria-label='Favorite' className='size-3.5 text-amber-400' /> : null}
      </span>
      <span className='flex min-w-0 flex-1 flex-col gap-0.5'>
        <span className='flex min-w-0 items-baseline gap-2'>
          <span className='w-[4.5rem] shrink-0 truncate text-[13px] font-medium' style={{ color: row.agentColor }}>
            {row.agent}
          </span>
        ...
        <span className='flex min-w-0 items-baseline gap-2 text-[13px] text-muted-foreground'>
          <span className='w-[4.5rem] shrink-0 truncate tabular-nums'>{timeLabel}</span>
