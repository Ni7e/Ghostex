    ...
    expect(
      getSessionCardTitleTooltip({
        session: {
          ...
          alias: 'Session 1',
          detail: 'OpenAI Codex',
          firstUserMessage: 'when using zmx/tmux/zellij as the persistence provider, keep the sidebar title readable',
          isPrimaryTitleTerminalTitle: true,
...
describe('SessionFloatingAgentIcon', () => {
  ...
  test('should show delayed send clock instead of a tag icon', () => {
    ...
    expect(markup).toContain('aria-label="Delayed Send: the prompt will be sent in 04:32"');
...
describe('SessionCardContent', () => {
  ...
  test('should show delayed send remaining time in the trailing timestamp slot', () => {
    ...
    expect(floatingMarkup).toContain('aria-label="Delayed Send: the prompt will be sent in 04:32"');
