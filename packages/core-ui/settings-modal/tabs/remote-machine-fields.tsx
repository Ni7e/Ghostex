...
export function applyRemoteMachineDraftPatch(
  ...
  return {
    ...
    transport:
      patch.transport !== undefined ? patch.transport : draft.transport,
    easyConnectCode:
      patch.easyConnectCode !== undefined
        ? patch.easyConnectCode
        : draft.easyConnectCode,
    easyConnectAddress:
      patch.easyConnectAddress !== undefined
        ? patch.easyConnectAddress
        : draft.easyConnectAddress,
    sshHost: patch.sshHost !== undefined ? patch.sshHost : draft.sshHost,
    sshIdentityFile:
      patch.sshIdentityFile !== undefined
        ? patch.sshIdentityFile
        : draft.sshIdentityFile,
    sshPassword:
      patch.sshPassword !== undefined ? patch.sshPassword : draft.sshPassword,
    sshPasswordSaved:
      patch.sshPasswordSaved !== undefined
        ? patch.sshPasswordSaved
        : draft.sshPasswordSaved,
    sshPort: patch.sshPort !== undefined ? patch.sshPort : draft.sshPort,
    sshUser: patch.sshUser !== undefined ? patch.sshUser : draft.sshUser,
    wslDistribution:
      patch.wslDistribution !== undefined
        ? patch.wslDistribution
        : draft.wslDistribution,
 ...
 */
export function readEasyConnectCodeInput(
  input: string,
...
export function easyConnectCodeDraftPatch(
  input: string,
  draft: RemoteMachineDraft,
): Partial<RemoteMachineDraft> {
  ...
  return {
    ...
    ...(reading.name && draft.name.trim().length === 0
      ? { name: reading.name }
      : {}),
    ...(reading.user ? { sshUser: reading.user } : {}),
    ...(reading.sshPort !== undefined
      ? { sshPort: String(reading.sshPort) }
      : {}),
  };
}
  ...
  const codeReading = easyConnect
    ? draft.easyConnectCode.trim().length > 0
      ...
      : draft.easyConnectAddress
        ? ({
            kind: "accepted",
            address: draft.easyConnectAddress,
            summary: "Paired through Easy Connect.",
          } as const)
        : ({ kind: "empty" } as const)
    : undefined;
  return (
    <FieldGroup className="settings-remote-machine-fields">
        ...
        >
          <div className="min-w-0 flex-1">
            ...
            <FieldDescription className="settings-remote-machine-field-description">
              Turn off to hide this machine from the sidebar without deleting
              it.
            </FieldDescription>
          </div>
          <Switch
      ...
      {easyConnect ? (
        <Field className="settings-remote-machine-field settings-remote-machine-easy-connect-code">
          ...
          <SettingsInput
            ...
            onChange={(event) =>
              onChange(
                easyConnectCodeDraftPatch(event.currentTarget.value, draft),
              )
            }
          ...
          <FieldDescription
            className={cn(
              ...
              codeReading?.kind === "rejected" &&
                "settings-remote-machine-code-rejected",
              codeReading?.kind === "accepted" &&
                "settings-remote-machine-code-accepted",
            )}
      ...
      {easyConnect ? null : (
        <Field className="settings-remote-machine-field">
          ...
          <SettingsInput
            ...
            maxLength={200}
            onChange={(event) =>
              onChange({ sshHost: event.currentTarget.value })
            }
      ...
      <Field className="settings-remote-machine-field">
        ...
        <div
          className={cn(
            ...
            !showPasswordSaveButton &&
              "settings-remote-machine-password-row-single",
          )}
        >
          <SettingsInput
            ...
            maxLength={500}
            onChange={(event) =>
              onChange({ sshPassword: event.currentTarget.value })
            }
            placeholder={
              draft.sshPasswordSaved ? "Saved in Keychain" : "SSH password"
            }
            type="password"
      ...
      <div className="settings-remote-machine-advanced">
        ...
        {advancedOpen ? (
          <div
            className="settings-remote-machine-advanced-fields"
            id={advancedId}
          >
            {easyConnect ? null : (
              <Field className="settings-remote-machine-field">
                ...
                <SettingsInput
                  ...
                  onChange={(event) =>
                    onChange({
                      sshPort: event.currentTarget.value.replace(
                        /[^0-9]/gu,
                        "",
                      ),
                    })
            ...
            <Field className="settings-remote-machine-field">
              ...
              <SettingsInput
                ...
                maxLength={500}
                onChange={(event) =>
                  onChange({ sshIdentityFile: event.currentTarget.value })
                }
              ...
              <FieldDescription className="settings-remote-machine-field-description">
                {identityDescription ??
                  "Optional. Use an SSH private key instead of a password."}
              </FieldDescription>
            </Field>
            <Field className="settings-remote-machine-field">
              <FieldLabel className="settings-remote-machine-field-label">
              ...
              <SettingsInput
                ...
                maxLength={120}
                onChange={(event) =>
                  onChange({ wslDistribution: event.currentTarget.value })
                }
                placeholder="Ubuntu-24.04"
                value={draft.wslDistribution}
              />
              <FieldDescription className="settings-remote-machine-field-description">
                Optional. Enter a distribution to use WSL2. Leave blank to
                follow Windows Environment on the remote computer, which
                defaults to native PowerShell.
              </FieldDescription>
            </Field>
          </div>
        ) : null}
...
export function formatRemoteMachineSshTarget(
  machine: RemoteMachineSettings,
): string {
  if (machine.transport === "easyConnect") {
    return machine.sshUser
      ? `Easy Connect · ${machine.sshUser}`
      : "Easy Connect";
  }
  const host = machine.sshUser
    ? `${machine.sshUser}@${machine.sshHost}`
    : machine.sshHost;
