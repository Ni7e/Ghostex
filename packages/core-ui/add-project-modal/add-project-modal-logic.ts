 ...
 * the path. On Windows machines the daemon answers `/` with its drive list;
 * entries carry the host's full drive paths so the client never invents them.
 */
...
export function addProjectSourceRowTitle(source: AddProjectSourceId): string {
  return source === "url"
    ? "Git URL"
    : `${addProjectSourceLabel(source)} repository`;
}

export function addProjectSourceRowDescription(
...
export function buildAddProjectSourceReadiness(
  ...
  for (const source of ADD_PROJECT_PROVIDER_SOURCES) {
    ...
    if (provider.status !== "available") {
      readiness[source] = {
        hint:
          provider.installHint?.trim() || ADD_PROJECT_PROVIDER_UNAVAILABLE_HINT,
        ready: false,
      };
      continue;
    }
...
export function sortAddProjectProviderSources(
  ...
  return [...ADD_PROJECT_PROVIDER_SOURCES].sort((left, right) => {
    ...
    return addProjectSourceLabel(left).localeCompare(
      addProjectSourceLabel(right),
    );
  });
}
...
export function addProjectRepositoryPlaceholder(
  source: AddProjectSourceId,
): string {
  if (source === "url") {
...
export function addProjectRepositoryActionLabel(
  source: AddProjectSourceId,
): string {
...
export function addProjectInitialBrowseQuery(
  machine: AddProjectMachineOption | null,
): string {
  const baseDirectory = machine?.addProjectBaseDirectory?.trim() ?? "";
  return baseDirectory.length === 0
    ? ADD_PROJECT_DEFAULT_BROWSE_PATH
    : baseDirectory;
}
...
export function addProjectSubmitActionLabel(
  input: AddProjectSubmitLabelInput,
): string {
...
export function addProjectEmptyStateMessage(
  input: AddProjectEmptyStateInput,
): string {
  ...
  if (input.cloneStep === "repository") {
    if (
      input.cloneSource === "url" ||
      input.cloneSource === "bitbucket" ||
      input.cloneSource === "azure-devops"
    ) {
