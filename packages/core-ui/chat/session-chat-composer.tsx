...
import {
  ...
  sessionChatSuggestionRowRadius,
} from "@/packages/shared/session-chat-presentation/composer-suggestions";
import {
...
} from "@/packages/shared/session-chat-controller/submission";
import {
...
import {
  ...
  type PendingDraft,
} from "./session-chat-draft-outbox";
import {
...
import {
  sessionChatCaretMovement,
  type SessionChatCaretMovement,
} from "./session-chat-caret-navigation";
...
import {
  SessionChatWorkingStrip,
  type SessionChatWorkingStripProps,
} from "./session-chat-working-strip";
...
import {
  GxserverRpcError,
  gxserverRpcErrorCode,
} from "@/packages/shared/gxserver-rpc-error";
...
import {
  SessionChatSendBlockedToaster,
  showSessionChatSendBlockedToast,
} from "./session-chat-send-blocked-toast";
...
import {
  ...
  recordSentSessionChatMessage,
} from "./session-chat-sent-history";
import {
...
export interface SessionChatComposerProps {
   ...
   * User: share the same Lexical editing behavior across desktop, web, and mobile while preserving the existing mobile controls and keyboard layout.
   * Lexical is the default even when the composer is mounted without SessionChatView.
   */
...
const MOBILE_SESSION_CHAT_PLACEHOLDER =
  "Tap ↑ to send or hold it to queue; use @ for files and $ for skills.";
...
>(function SessionChatComposer(
  ...
  const parkedDraftRef = useRef(
    readStoredSessionChatDraftEntry(sessionKey)?.parked === true,
  );
  ...
  useEffect(
    () =>
      subscribeDraftSaveStatus(() =>
        setSaveStatus(draftSaveStatus(sessionKey)),
      ),
    [sessionKey],
  );
  useEffect(() => {
    if (sessionKey && draftSync?.canSync)
      registerDraftWriter(sessionKey, (entry) =>
        draftSync.push(entry.content, entry.version),
      );
  }, [sessionKey, draftSync?.canSync, draftSync?.push]);
  ...
  const setHistory = (
    next:
      ...
      | ((current: SessionChatComposerHistory) => SessionChatComposerHistory),
  ): void => {
    historyRef.current =
  ...
  const referenceInteractions = useSessionChatReferenceInteractions(draft);
  const [clickedImageReference, setClickedImageReference] = useState<{
    path: string;
    caret: number | null;
    draft: string;
  } | null>(null);
  /**
   * CDXC:SessionChat 2026-09-09 DECISION:
   * User: hovering or clicking an image reference or placing the caret in it outlines its preview image in white across desktop, web, and mobile chat inputs.
   */
  const activeImagePath = useMemo(() => {
    if (referenceInteractions.hoveredImagePath !== null)
      return referenceInteractions.hoveredImagePath;
    if (
      clickedImageReference?.draft === draft &&
      clickedImageReference.caret === caret
    ) {
      return clickedImageReference.path;
    }
    if (caret === null) return null;
    for (const match of draft.matchAll(LINKED_IMAGE_REFERENCE_PATTERN)) {
    ...
    caret,
    clickedImageReference,
    draft,
    referenceInteractions.hoveredImagePath,
  ]);
  const [pastedImages, setPastedImages] = useState<
    readonly PastedImagePreview[]
  >([]);
  ...
  const [sendErrorCode, setSendErrorCode] =
    useState<GxserverRpcErrorCode | null>(null);
  ...
  const [incomingDraft, setIncomingDraft] = useState<SessionChatDraft | null>(
    null,
  );
  ...
  const traceDraft = (
    ...
    diagnosticLogRef.current?.(`sessionChat.draft.${phase}`, {
      ...
      editor: sessionChatDraftFingerprint(
        getInputApi()?.getValue() ?? draftRef.current,
      ),
      stored: stored
        ? {
            ...sessionChatDraftFingerprint(stored.text),
  ...
  useEffect(() => {
    ...
    diagnosticLogRef.current?.("sessionChat.composerEmptyChanged", {
      empty: draftEmpty,
    });
  }, [draftEmpty]);
  useEffect(
  ...
  const {
    ...
    () =>
      composerSuggestions({
        draft,
        caret: caret ?? draft.length,
        slashCommands,
        skills,
        files,
        skillsLoading,
        skillsError,
        canRequestSkills: !!onRequestSkills,
        filesLoading,
        slashDismissed,
        skillDismissed,
        fileDismissed,
  ...
  const canRelease = (requireDurableSelection = true): boolean => {
    ...
    const allowed =
      ...
      releasePreparedVersionRef.current ===
        JSON.stringify(draftVersionRef.current) &&
    ...
    if (allowed && sessionKey && input) {
      try {
        saveSessionChatComposerSelection(sessionKey, {
          text: input.getValue(),
          ...input.getSelection(),
        });
        if (
          requireDurableSelection &&
          !isSessionChatComposerSelectionDurable(sessionKey)
        )
          return false;
      } catch {
        return false;
  ...
  useEffect(() => {
    const input = getInputApi();
    const selection = readSessionChatComposerSelection(
      sessionKey,
      draftRef.current,
    );
    if (input && selection) {
  ...
  const prepareRelease = async (): Promise<PendingDraft[] | null> => {
    ...
    if (
      pastedImages.some(
        (image) => !linkedImageReferenceHrefs(content).includes(image.path),
      )
    )
      return null;
    const stored = readStoredSessionChatDraftEntry(sessionKey);
    ...
    try {
      ...
      if (
        !canRelease(false) ||
        readStoredSessionChatDraftEntry(sessionKey)?.text !== content
      )
        return null;
      saveSessionChatComposerSelection(sessionKey, {
        text: content,
  ...
  useImperativeHandle(ref, () => ({
    ...
    prepareRelease,
    attachDroppedFiles: (data: DataTransfer): boolean =>
      consumeDroppedAttachments(data),
    appendText: (text: string): boolean => {
      if (text === "") {
        return false;
      }
    ...
    handoffDraft: async () => {
      ...
      if (
        ...
        throw new Error(
          "The draft is still being prepared. It has been kept in Chat.",
        );
      }
      const entry = readStoredSessionChatDraftEntry(sessionKey);
      const saved =
        entry?.text === content && entry.version
          ? entry
          : persistComposerDraft(content);
      const version = saved.version!;
      pendingDraftTransfersRef.current++;
      try {
        await flushDraftSaves(sessionKey);
        await draftSync.push(content, version);
        if (
          ...
          throw new Error(
            "The draft changed during transfer. It has been kept in Chat.",
          );
        }
    ...
    receiveDraftHandoff: async (handoff) => {
      if (!sessionKey || !draftSync?.canSync)
        throw new Error("Draft saving is unavailable.");
      if (!getInputApi()) throw new Error("The Chat input is not ready.");
      if (
        handoff.draftVersion &&
        draftSync.synced?.consumedDrafts?.some(
          (receipt) =>
            receipt.draftId === handoff.draftVersion!.draftId &&
            receipt.revision >= handoff.draftVersion!.revision,
        )
      )
        return;
      const current = getInputApi()?.getValue() ?? draftRef.current;
      const stored = readStoredSessionChatDraftEntry(sessionKey);
      if (handoff.draftVersion)
        preserveDraftRevision({
          sessionKey,
      ...
      if (disposition === "conflict") {
        ...
        setIncomingDraft({
          content: handoff.content,
          version: handoff.draftVersion,
          originClientId: "gxserver-draft-handoff",
          updatedAt: new Date().toISOString(),
        });
        return;
      }
      parkedDraftRef.current = false;
      draftVersionRef.current = handoff.draftVersion;
      const accepted = writeStoredSessionChatDraft(
        sessionKey,
        handoff.content,
        undefined,
        handoff.draftVersion,
      );
      draftVersionRef.current = accepted.version;
      composerTouchedRef.current = true;
      draftRef.current = handoff.content;
      setDraft(handoff.content);
      setCaret(handoff.content.length);
      getInputApi()?.applyValue(handoff.content, handoff.content.length);
      await flushDraftSaves(sessionKey);
    },
    insertSavedPrompt: (text: string): boolean => {
      const input = getInputApi();
    ...
    copyClipboard: (data, cut): boolean => {
      ...
      data.setData(
        "text/plain",
        input.getValue().slice(selection.start, selection.end),
      );
      playCopySound();
      expandComposer();
      input.focus();
   ...
   * User: Option+Enter, Option-click on Send, or Send's right-click "Compact & Send" sends /compact to the terminal first, then queues the text written in chat.
   */
  const send = (
    ...
    traceDraft("sendBegin", {
      ...
      storedPrefixOfSent: text.startsWith(
        readStoredSessionChatDraft(sessionKey),
      ),
    });
    ...
    const sendRequest = deliverChatSubmission({
      ...
      push: draftSync?.canSync
        ? async (value, version) => {
            traceDraft("sendSaveBegin", {
              sent: sessionChatDraftFingerprint(value),
            });
            await draftSync.push(value, version);
            traceDraft("sendSaveAcknowledged", {
              sent: sessionChatDraftFingerprint(value),
            });
          }
        : undefined,
      ...
      phase: (phase) => {
        sendPhase = phase;
        if (phase === "deliverMessage")
          traceDraft("deliveryBegin", {
            sent: sessionChatDraftFingerprint(text),
          });
      },
    });
    ...
    void sendRequest
        ...
        (error: unknown) => {
          if (gxserverRpcErrorCode(error) !== "sendCancelled") {
            diagnosticLogRef.current?.("sessionChat.sendFailure", {
              ...
              endpoint:
                error instanceof GxserverRpcError ? error.endpoint : null,
              sent: sessionChatDraftFingerprint(text),
      ...
      .catch((error: unknown) => {
        console.error(
          "[session-chat] Could not update local composer state after sending.",
          error,
        );
      })
      .finally(() => {
  ...
  const canQueueDraft =
    queueCapabilities?.canQueue === true && draft.trim() !== "";
    ...
    void (async () => {
      try {
        ...
        try {
          ...
          console.error(
            "[session-chat] Could not update local composer state after queueing.",
            error,
          );
        }
      } catch {
        restoreComposerText(text);
        setSendError(
          "The prompt could not be queued. Your draft was restored.",
        );
      } finally {
        pendingComposerOperationsRef.current -= 1;
        pendingDraftTransfersRef.current -= 1;
  ...
  */
  ...
  const handleSendClick = (event: ReactMouseEvent<HTMLElement>): void => {
    cancelSendLongPress();
  ...
  const sendButtonLabel = canQueueDraft
    ? `Send (hold to queue, ${formatSidebarHotkeyLabel("alt")}-click or right-click for Compact & Send)`
    : "Send";
  ...
  const pushDraftIfChanged = (): void => {
    if (
      ...
      return;
    const content = getInputApi()?.getValue() ?? draftRef.current;
    if (content === "" && pendingDraftTransfersRef.current > 0) return;
    const stored = readStoredSessionChatDraftEntry(sessionKey);
    if (
      !draftVersionRef.current ||
      stored?.text !== content ||
  ...
  useEffect(() => {
    ...
    if (sessionKey)
      retireDraftRecovery(sessionKey, syncedDraft.consumedDrafts ?? []);
    if (parkedDraftRef.current || syncedDraft.parked) {
    ...
    const retired =
      ...
      syncedDraft.consumedDrafts?.some(
        (receipt) =>
          receipt.draftId === stored.version?.draftId &&
          receipt.revision >= stored.version.revision,
      );
    const staleBase =
      stored?.version &&
      ...
      (syncedDraft.version.revision > stored.version.revision ||
        (syncedDraft.version.revision === stored.version.revision &&
          syncedDraft.content !== composerText));
    ...
    // A restart can return this draft under the old outbox writer ID; its stable draft identity still lets an untouched composer recover a newer revision quietly.
    const recovered =
      retired ||
      (!composerTouchedRef.current &&
        (syncedDraft.originClientId === draftClientId ||
          (stored?.version &&
            syncedDraft.version?.draftId === stored.version.draftId)))
  ...
  const acceptIncomingDraft = (): void => {
    ...
    const existing = readStoredSessionChatDraftEntry(sessionKey);
  ...
  const insertReference = (reference: string): void => {
    ...
    const { text: next, caret } = insertChatReference(
      current,
      reference,
      start,
      end,
  ...
  const appendFileReferences = (paths: readonly string[]): void => {
    ...
    const references = paths
      .map((path, index) => `[File #${firstIndex + index}](${path})`)
      .join("\n");
    const separator =
   ...
   */
  const insertNativePathReferences = async (
      ...
      .then(async ({ directories, files }) => {
        ...
        if (attachmentFiles.length > 0 || directories.length > 0) {
          appendFileReferences(
            await uploadSessionChatDroppedAttachments(
              { directories, files: attachmentFiles },
              onAttachFile,
            ),
          );
        }
  ...
  const removePastedImage = (image: PastedImagePreview): void => {
    ...
    if (matchIndex < 0) {
      ...
      setPastedImages((images) =>
        images.filter((entry) => entry.id !== image.id),
      );
      return;
    }
    ...
    void (async () => {
      for (const file of files) {
        ...
        try {
          ...
          const { path, dataUrl } = await saveSessionChatImageFile(
            file,
            onPasteImage,
          );
  ...
  const attachFromNativePicker = (selection?: "files" | "folders"): void => {
    ...
    void (async () => {
      try {
        await insertNativePathReferences(
          (await onPickPaths?.(selection)) ?? [],
        );
      } catch (error) {
  ...
  const handleSlashKeyDown = (event: SessionChatComposerKeyEvent): boolean => {
    ...
    if (event.key === "ArrowUp" || event.key === "ArrowDown") {
      ...
      setSlashIndex((current) => {
        ...
        return (
          (currentIndex + delta + slashMatches.length) % slashMatches.length
        );
      });
      return true;
    }
  ...
  const completeMention = (replacement: string): void => {
    const completion = completeComposerMention(
      draft,
      caret ?? draft.length,
      replacement,
    );
  ...
  const completeFileMention = (path: string): void => {
    completeMention(
      `${sessionChatFileMention(path, nextFileReferenceIndex(draft))} `,
    );
  };
  ...
  const handleSkillKeyDown = (event: SessionChatComposerKeyEvent): boolean => {
    ...
    if (event.key === "ArrowUp" || event.key === "ArrowDown") {
      ...
      setSkillIndex((current) => {
        ...
        return (
          (currentIndex + delta + skillMatches.length) % skillMatches.length
        );
      });
      return true;
    }
  ...
  const handleKeyDown = (event: SessionChatComposerKeyEvent): void => {
    ...
    */
    ...
    const canRecall =
      ...
      !event.ctrlKey &&
      !event.shiftKey &&
      (!event.altKey || draftRef.current.trim() === "");
    if (
      canRecall &&
      event.key === "ArrowUp" &&
      (draftRef.current.trim() === "" || history.index !== null)
    ) {
      ...
      if (recalled) {
        ...
        setSlashDismissed(true);
        const recalledTrigger = detectSessionChatComposerTrigger(
          recalled.draft,
          recalled.draft.length,
        );
  ...
  */
  ...
  const showStopButton =
    (isWorking || stopButtonCoolingDown) && !hasSendableDraft;
  const inputPlaceholder =
    placeholder ??
    (sendOnEnter
      ? DESKTOP_SESSION_CHAT_PLACEHOLDER
      : MOBILE_SESSION_CHAT_PLACEHOLDER);
  /** CDXC:SessionChat 2026-09-14 DECISION: User: an empty, collapsed composer shows only the first line of its placeholder. */
  const visiblePlaceholder = collapsed
    ? inputPlaceholder.split(/\r?\n/, 1)[0]
    : inputPlaceholder;
  ...
  return (
    <>
      <SessionChatSendBlockedToaster
        theme={theme}
        toasterId={sendBlockedToasterId}
      />
      {maximized ? (
        <div
          aria-hidden="true"
      ...
      >
        ...
        {!maximized &&
          (workingStatusContainer ? (
            createPortal(
              <SessionChatWorkingStrip
                {...(workingStatus ?? { working: isWorking, activity: null })}
              />,
              workingStatusContainer,
            )
          ) : (
            ...
            >
              ...
              {slashMatches.map((command, index) => (
                <button
                  ...
                  style={{
                    borderRadius: sessionChatSuggestionRowRadius(
                      index,
                      slashMatches.length,
                    ),
                  }}
        ...
        {sendError && sendErrorCode === "composerNotReady" ? (
          <SessionChatComposerNotReadyNotice
            ...
            {...(hostActions?.onSwitchToTerminal
              ? { onOpenTerminal: hostActions.onSwitchToTerminal }
              : {})}
          />
        ) : sendError ? (
        ...
        {incomingDraft ? (
          <SessionChatDraftConflict
            draft={incomingDraft}
            onUse={acceptIncomingDraft}
            onDismiss={dismissIncomingDraft}
          />
        ) : null}
        <div
          className={cn(
          ...
          onPointerDownCapture={() => {
            expandComposer();
            referenceInteractions.cancelImageOpen();
          }}
          onMouseDownCapture={(event) => {
            if (event.detail > 1) referenceInteractions.cancelImageOpen();
          }}
          onDoubleClickCapture={referenceInteractions.cancelImageOpen}
          onPointerOver={(event) =>
            referenceInteractions.hoverReference(event.target)
          }
          onPointerOut={(event) =>
            referenceInteractions.hoverReference(event.relatedTarget)
          }
          onPointerLeave={() => referenceInteractions.hoverReference(null)}
          onKeyDownCapture={() => {
            expandComposer();
            referenceInteractions.cancelImageOpen();
          }}
          onClick={(event) => {
            referenceInteractions.clickReference(event);
            const reference =
              event.target instanceof Element
                ? event.target.closest<HTMLElement>(
                    "[data-ghostex-reference-path]",
                  )
                : null;
            const path = reference?.dataset.ghostexReferencePath;
            setClickedImageReference(
              path
                ? {
                    path,
                    caret: getInputApi()?.getSelection().end ?? caret,
                    draft,
                  }
                : null,
            );
        ...
        >
          ...
          >
            <ContextMenuTrigger
              className="ghostex-chat-composer-row flex min-w-0 select-text items-end gap-2 pb-1.5"
              onPointerDownCapture={(event) =>
                referenceInteractions.captureReferenceContext(event.target)
              }
            ...
            <ContextMenuContent
              className="ghostex-session-chat-popup"
              data-chat-theme={theme}
            >
              {referenceInteractions.contextReference !== null ? (
                <ContextMenuGroup>
                  <SessionChatReferenceMenuItems
                    href={referenceInteractions.contextReference}
                  />
                </ContextMenuGroup>
              ) : (
                <>
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          ...
                          const images = files.filter(
                            (file) =>
                              isImageFile(file) && onPasteImage !== undefined,
                          );
                ...
                <SessionChatComposerActions
                  ...
                  onMenuClosed={() => {
                    ...
                    if (
                      ...
                      active.closest(".ghostex-chat-composer-toolbar")
                    ) {
                      getInputApi()?.focus();
                    }
                  }}
                  ...
                  /Linux/i.test(navigator.platform)
                    ? {
                        onAttachFolders: () =>
                          attachFromNativePicker("folders"),
                      }
                    : {})}
                />
                {showStopButton ? (
                  ...
                  <AppTooltip content={sendButtonLabel} side="top">
                    <span className="inline-flex">
                        ...
                        onOpenChange={(open, details) => {
                          ...
                          if (
                            ...
                            (details.event.type.startsWith("touch") ||
                              (details.event.type === "contextmenu" &&
                                sendPointerTypeRef.current === "touch"))
                          ) {
                            details.cancel();
                            return;
                          }
                      ...
                      >
                        <ContextMenuTrigger
                          render={
                            <Button disabled={!hasSendableDraft} size="icon" />
                          }
                          ...
                          className={cn(
                            "ghostex-chat-send-button size-6",
                        ...
                        >
                          ...
                          >
                            Compact &amp; Send
                            <ContextMenuShortcut>
                              {formatSidebarHotkeyLabel("alt+enter")}
                            </ContextMenuShortcut>
                          </ContextMenuItem>
                        </ContextMenuContent>
                      </ContextMenu>
