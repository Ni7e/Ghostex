...
import { SESSION_CHAT_DEFAULT_COMMAND_CATALOG, classifySessionChatSend } from '../session-chat-send-classification';
...
import { selectSessionChatViewState, sessionChatTranscriptStatusAfterState } from '../session-chat-view-state';
...
import {
  ...
  SessionChatStreamPosition,
  UseSessionChatOptions,
  UseSessionChatResult,
  isAheadOf,
  normalizedSessionChatText,
  ...
  */
  ...
  useLayoutEffect(() => {
    ...
    */
    ...
    // an empty transcript between mount and its subscription's first frame.
    const cachedSnapshot = transport.getCachedSnapshot?.();
    if (cachedSnapshot) {
  ...
  return {
    ...
    error,
    hasMore,
    earlierPageCursor: beforeOffsetRef.current,
    interrupt,
    lifecycle,
    loadEarlier,
    loadingEarlier,
