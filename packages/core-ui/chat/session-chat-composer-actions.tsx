...
} from "@/packages/shared/session-chat-presentation/actions";
import {
  IconClock,
  IconClockCheck,
  IconDots,
...
import {
  formatSessionTerminalTailPreview,
  useSessionTerminalTail,
} from "./use-session-terminal-tail";
...
export function SessionChatComposerActions({
  ...
  */
    ...
    ...(onShowStashedPrompts
      ? [
          withShortcut(
            ...
            hostActionShortcut("stashedPrompts"),
          ),
        ]
      : []),
  ].join("\n");
  ...
  const openSavedPromptsFromContextMenu = (
  ...
  const summaryShortcut = formatSidebarHotkeyLabel(
  ...
  */
  const [inputAction, setInputAction] = useState<SessionChatHostAction | null>(
    null,
  );
  ...
  const { toolbarRef, isOverflowed, optionsOverflowed } =
    useSessionChatComposerOverflow(inputAction !== null);
    ...
    action.items ? (
      ...
      */
      <DropdownMenuSub key={action.id}>
        ...
        <DropdownMenuSubContent className="ghostex-session-chat-popup w-56">
          {action.items.map((item) => (
            <DropdownMenuItem
              key={item.id}
              onClick={() => hostActions?.onAction?.(action.id, item.id)}
            >
      ...
      <DropdownMenuItem key={action.id} onClick={() => runHostAction(action)}>
        {hostActionIcon(action.id)}
        {action.label}
        {showShortcutLabels && action.shortcut ? (
  ...
  */
  const delayedHostAction = hostActionList.find(
    (action) => action.id === "delayedActions",
  );
  const closeAfterDoneHostAction = hostActionList.find(
    (action) => action.id === "closeAfterDone",
  );
  const splitRightHostAction = hostActionList.find(
    (action) => action.id === "splitSessionRight",
  );
  ...
  const agentHostActions = foldedHostActions.filter((action) =>
    AGENT_HOST_ACTION_IDS.has(action.id),
  );
  const otherHostActions = foldedHostActions.filter(
    (action) => !AGENT_HOST_ACTION_IDS.has(action.id),
  );
  ...
  const summaryMenuItem = onToggleSummary ? (
    ...
    >
      ...
      Summary mode
      {showShortcutLabels ? (
        <DropdownMenuShortcut>{summaryShortcut}</DropdownMenuShortcut>
      ) : null}
    </DropdownMenuCheckboxItem>
  ) : null;
    ...
    onDelayedActions || delayedHostAction ? (
      <DropdownMenuItem
        ...
        onClick={
          onDelayedActions ??
          (delayedHostAction
            ? () => runHostAction(delayedHostAction)
            : undefined)
        }
      >
        ...
        {onDelayedActions
          ? "Delayed actions"
          : (delayedHostAction?.label ?? "Delayed actions")}
        {showShortcutLabels && delayedHostAction?.shortcut ? (
          <DropdownMenuShortcut>
            {delayedHostAction.shortcut}
          </DropdownMenuShortcut>
        ) : null}
      </DropdownMenuItem>
    ) : null;
  const closeAfterDoneMenuItem = closeAfterDoneHostAction
    ? hostActionMenuItem(closeAfterDoneHostAction)
    : null;
  ...
  const accountSubmenu = renderAccountMenu ? (
    <DropdownMenuSub>
      ...
      <DropdownMenuSubContent
        className="ghostex-session-chat-popup gx-account-submenu"
        sideOffset={8}
      >
  ...
  const otherHostMenuSection =
    otherHostActions.length > 0 ? (
      <DropdownMenuGroup>
        {otherHostActions.map(hostActionMenuItem)}
      </DropdownMenuGroup>
    ) : null;
  ...
  const hostMenuSections = (precededByItems: boolean) => (
    <>
      ...
      {otherHostMenuSection ? (
        <>
          {precededByItems || agentMenuSection ? (
            <DropdownMenuSeparator />
          ) : null}
          {otherHostMenuSection}
        </>
  ...
  */
  const { refreshNow: refreshTerminalTail, tail: terminalTail } =
    useSessionTerminalTail(onReadTerminalTail);
  const terminalReadiness =
    terminalTail?.captured && terminalTail.composerState !== "unknown"
      ? terminalTail.composerState
      : null;
  const terminalTailPreview =
    terminalTail?.captured === true
      ? formatSessionTerminalTailPreview(terminalTail.lines)
      : "";
  const switchViewTitle = withShortcut(
    "Click to Switch to Terminal View",
    hostActions?.switchViewShortcut,
  );
  const switchViewNotReadyReason =
    terminalReadiness === "notReady" ? (terminalTail?.reason ?? null) : null;
  const switchViewTooltip: ReactNode = (
    <div className="flex min-w-0 flex-col gap-1.5">
      ...
      {switchViewNotReadyReason ? (
        <div className="opacity-70">{switchViewNotReadyReason}</div>
      ) : null}
  ...
  const switchViewButton = hostActions ? (
    <AppTooltip
      ...
      {...(terminalTailPreview.length > 0
        ? { contentStyle: { maxWidth: "min(92vw, calc(46rem + 200px))" } }
        : {})}
    >
      <span
        className="inline-flex"
        onFocus={refreshTerminalTail}
        onMouseEnter={refreshTerminalTail}
      >
        <Button
          ...
          variant="ghost"
          {...(terminalReadiness
            ? { "data-terminal-ready": terminalReadiness }
            : {})}
        >
  ...
  return (
    <div className="ghostex-chat-composer-toolbar" ref={toolbarRef}>
      ...
      >
        <AppTooltip
          content={withShortcut(
            "More actions",
            hostActions?.moreActionsShortcut,
          )}
        >
          <DropdownMenuTrigger
            render={
        ...
        <DropdownMenuContent
          align="end"
          className="ghostex-session-chat-popup w-60"
          side="top"
        >
          ...
          <DropdownMenuGroup>
            ...
            {splitRightHostAction
              ? hostActionMenuItem(splitRightHostAction)
              : null}
              ...
              >
                <span className="ghostex-chat-composer-menu-indicator relative inline-flex">
                  ...
                  {sessionNoteHasText ? (
                    <span
                      aria-hidden="true"
                      className="ghostex-chat-session-note-presence-dot"
                    >
                      1
                    </span>
                  ) : null}
              ...
              <AppTooltip content={stashTooltip} side="left">
                <DropdownMenuItem
                  ...
                  disabled={
                    hasSendableDraft
                      ? false
                      : onShowStashedPrompts === undefined
                  }
            ...
            {isOverflowed("attach") && onAttach ? (
              onAttachFolders ? (
                <DropdownMenuSub>
                  ...
                  <DropdownMenuSubContent className="ghostex-session-chat-popup">
                    <DropdownMenuItem onClick={onAttach}>
                      Images or files…
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={onAttachFolders}>
                      Folders…
                    </DropdownMenuItem>
                  </DropdownMenuSubContent>
                </DropdownMenuSub>
      ...
      {onToggleSummary ? (
        <span
          data-composer-action="summary"
          data-overflowed={isOverflowed("summary")}
        >
      ...
      {onSessionNote ? (
        <span
          data-composer-action="note"
          data-overflowed={isOverflowed("note")}
        >
          <AppTooltip
            content={withShortcut(
              "Session note",
      ...
      {onStash ? (
        <span
          data-composer-action="stash"
          data-overflowed={isOverflowed("stash")}
        >
          <AppTooltip content={stashTooltip}>
            <span className="ghostex-chat-stash-control relative inline-flex">
              <Button
                ...
                className="ghostex-chat-footer-control rounded-full"
                disabled={
                  hasSendableDraft ? false : onShowStashedPrompts === undefined
                }
                onClick={stashCurrentPrompt}
                onContextMenu={openSavedPromptsFromContextMenu}
                size="icon-sm"
      ...
      {onAttach ? (
        <span
          ...
          data-overflowed={isOverflowed("attach")}
        >
          <AppTooltip
            content={withShortcut(
              "Attach a file or folder",
              hostActionShortcut("attachPath"),
            )}
          >
            <span className="inline-flex">
              {onAttachFolders ? (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent className="ghostex-session-chat-popup">
                    <DropdownMenuItem onClick={onAttach}>
                      Images or files…
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={onAttachFolders}>
      ...
      <span
        data-composer-action="maximize"
        data-overflowed={isOverflowed("maximize")}
      >
      ...
      {switchViewButton ? (
        <span
          data-composer-action="terminal"
          data-overflowed={isOverflowed("terminal")}
        >
          {switchViewButton}
        </span>
      ) : null}
