...
// GPUI chat reads the same preview through its own tail controller, and the
// native chat runtime must stay independent of React, so the formatter is shared.
import { formatSessionTerminalTailPreview } from '@/packages/shared/session-chat-presentation/terminal-tail';

export { formatSessionTerminalTailPreview };
