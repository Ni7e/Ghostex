  ...
  const assets = readdirSync(assetDir, { withFileTypes: true })
    .filter(
      (entry) =>
        ...
        (entry.name.endsWith('.tar.gz') || entry.name.endsWith('.tgz'))
    )
    .map((entry) => {
...
export function verifyPublishedComponent({ component, release }) {
  ...
  for (const [platform, sealed] of Object.entries(component.platforms ?? {})) {
    ...
    if (sealed.sha256SidecarName && !remoteByName.has(sealed.sha256SidecarName)) {
      throw new Error(`Component tag ${tag} is missing ${sealed.sha256SidecarName} for ${platform}. Fix: ${fix}`);
    }
  }
  return component;
}
...
function reusePublishedAsset({ assetDir, component, componentVersion, platform, repo, requireSha256Sidecar = false }) {
  ...
  const downloadArgs = ['release', 'download', tag, '--repo', repo, '--pattern', assetName];
  if (requireSha256Sidecar) downloadArgs.push('--pattern', sidecarName);
  downloadArgs.push('--dir', assetDir, '--clobber');
  runGh(downloadArgs);
...
function createReleaseIfMissing({ component, componentVersion, repo, tag }) {
  ...
  if (!release.exists) {
    throw new Error(
      `Could not create component release ${tag}: ${(result.stderr || result.stdout || 'unknown error').trim()}`
    );
  }
}
...
function uploadAssetIdempotently({ asset, repo, tag }) {
  ...
  if (remote && normalizedRemoteDigest(remote) === asset.sha256 && Number(remote.size) === asset.sizeBytes) {
    return;
  }
  throw new Error(
...
async function main() {
  ...
  if (options['release-state'] && !options.dryRun)
    throw new Error('--release-state is test-only and requires --dry-run');
