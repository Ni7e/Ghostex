...
const FIXTURE_FILES = {
   ...
   * no product (CDXC:Release 2026-09-01 — it is an opt-in
  ...
  'packages/components/ui/button.tsx': 'export const Button = () => null;\n',
  'packages/paths/src/lib.rs': 'pub fn home() {}\n',
  ...
  'packages/components/utils.ts': 'export const cn = () => "";\n',
  ...
  'package.json': `${JSON.stringify(
    {
      ...
      private: true,
      scripts: { 'release:test': 'vitest run --config tooling/release-gpui/vitest.release.config.ts' },
      version: '7.7.0',
    },
    null,
    2
  ...
  'tooling/release-gpui/create-deterministic-tar.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/fingerprint.mjs': '// fingerprint\n',
  'tooling/release-gpui/install-gxserver-wsl.ps1': '# wsl install\n',
  'tooling/release-gpui/linux-deb.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/linux-rpm.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/linux-stage.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/linux-tar.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/macos-finalize.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/macos-notary.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/macos-prerequisite.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/macos.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/on-demand-manifest.mjs': '// manifest v2\n',
  'tooling/release-gpui/patches/code-server-ripgrep-target-validation.patch': '--- a\n+++ b\n',
  'tooling/release-gpui/prepare-references.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/prepare-sparkle.sh': '#!/usr/bin/env bash\n',
  'tooling/release-gpui/prepare-zig.ps1': '# zig\n',
  'tooling/release-gpui/product-inputs.mjs': '// products\n',
  'tooling/release-gpui/publish-component.mjs': '// components\n',
  'tooling/release-gpui/verify-code-server-archive.mjs': '// verify\n',
  'tooling/release-gpui/windows-update-feed.mjs': '// feeds\n',
