...
function resolveWindowsProgramFilesPaths() {
  ...
  if (result.status !== 0 || !windowsPath) {
    throw new Error(result.stderr?.trim() || 'Windows did not report its Program Files directory.');
  }
  if (!isWsl) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      if (Number.isInteger(holderPid) && holderPid > 0 && processIsAlive(holderPid)) {
        throw new Error(`Another "bun run start" (pid ${holderPid}) is already rebuilding the GPUI app.`);
      }
      rmSync(localStartLockFile, { force: true });
    }
  }
...
function reexecUnderLocalStartLock() {
  ...
  */
  ...
  const result = spawnSync(lockCommand, [...lockArgs, process.execPath, scriptPath, ...process.argv.slice(2)], {
    cwd: repoRoot,
...
function ensureReferenceCheckout({ name, requiredRelativePath }) {
  ...
  if (!existsSync(expectedRequiredPath)) {
    if (pathExistsWithoutFollowingFinalSymlink(expectedPath) && !dependencySubmoduleIsUninitialized(name)) {
      throw new Error(
...
function dependencySubmoduleIsUninitialized(name) {
  ...
  const result = spawnSync('git', ['-c', `safe.directory=${repoRoot}`, 'submodule', 'status', '--', relativePath], {
    cwd: repoRoot,
    encoding: 'utf8',
...
function initializeDependencySubmodule(name) {
  const relativePath = path.join('.dependencies', name);
  run('git', ['-c', `safe.directory=${repoRoot}`, 'submodule', 'update', '--init', '--depth=1', '--', relativePath], {
    env: startEnvironment,
...
function windowsPathForHostPath(hostPath) {
  ...
  const windowsPath = result.stdout?.trim();
  if (result.status !== 0 || !windowsPath) {
    throw new Error(result.stderr?.trim() || `Could not map the WSL path ${hostPath} into Windows.`);
  }
  return windowsPath;
}
...
async function stopRunningGxserverControlPlaneBeforeLaunch(stagedAppPath) {
  ...
  if (!expectedBuildIdentity) {
    console.warn(
      'The built GPUI app has no bundled gxserver build identity; stopping any running control plane anyway.'
    );
  }
...
function summarizeCodesignLog(logText) {
  ...
  if (
    lines.some((line) => line.includes('valid on disk')) &&
    lines.some((line) => line.includes('satisfies its Designated Requirement'))
  ) {
