...
#[cfg(test)]
mod tests {
    use super::{ffi, ghosttykit_availability, ghosttykit_paths};
    use std::{
        ffi::{c_char, c_int, c_void},
    ...
    #[test]
    fn ghosttykit_paths_resolve_to_repo_local_bundle() {
        let paths = ghosttykit_paths(env!("CARGO_MANIFEST_DIR"));

        assert!(paths.header.ends_with("Headers/ghostty.h"));
        assert!(paths.archive.ends_with("ghostty-internal.a"));
        assert!(paths.header.starts_with(env!("CARGO_MANIFEST_DIR")));
        assert!(paths.archive.starts_with(env!("CARGO_MANIFEST_DIR")));
        assert!(paths.header.components().any(|component| {
            component.as_os_str() == Path::new("GhosttyKit.xcframework").as_os_str()
        }));
    }

    #[test]
    fn ghosttykit_availability_detects_local_header_and_archive() {
        let availability = ghosttykit_availability(env!("CARGO_MANIFEST_DIR"));

        assert!(availability.header_exists);
        assert!(availability.archive_exists);
        assert!(availability.is_complete());
    }

    #[test]
    fn abi_constants_match_ghostty_header_discriminants() {
        assert_eq!(ffi::GHOSTTY_SUCCESS, 0);
        assert_eq!(ffi::GHOSTTY_PLATFORM_INVALID, 0);
        assert_eq!(ffi::GHOSTTY_PLATFORM_MACOS, 1);
        assert_eq!(ffi::GHOSTTY_PLATFORM_IOS, 2);
        assert_eq!(ffi::GHOSTTY_SURFACE_CONTEXT_WINDOW, 0);
        assert_eq!(ffi::GHOSTTY_SURFACE_CONTEXT_TAB, 1);
        assert_eq!(ffi::GHOSTTY_SURFACE_CONTEXT_SPLIT, 2);
    ...
    #[test]
    fn opaque_handles_and_platform_structs_match_pointer_shape() {
        assert_eq!(size_of::<ffi::ghostty_app_t>(), size_of::<*mut c_void>());
        assert_eq!(size_of::<ffi::ghostty_config_t>(), size_of::<*mut c_void>());
        assert_eq!(
            size_of::<ffi::ghostty_surface_t>(),
            size_of::<*mut c_void>()
        );
        assert_eq!(
            size_of::<ffi::ghostty_platform_macos_s>(),
            size_of::<*mut c_void>()
        );
        assert_eq!(
            align_of::<ffi::ghostty_platform_macos_s>(),
            align_of::<*mut c_void>()
        );
        assert_eq!(
            size_of::<ffi::ghostty_platform_u>(),
            size_of::<*mut c_void>()
        );
    }

    #[test]
    fn surface_config_struct_preserves_c_layout_on_64_bit_targets() {
        assert_eq!(size_of::<ffi::ghostty_platform_e>(), size_of::<c_int>());
        assert_eq!(
            size_of::<ffi::ghostty_surface_context_e>(),
            size_of::<c_int>()
        );
        assert_eq!(
            size_of::<ffi::ghostty_env_var_s>(),
            size_of::<*const c_char>() * 2
        );

        #[cfg(target_pointer_width = "64")]
        {
            assert_eq!(size_of::<ffi::ghostty_string_s>(), 24);
            assert_eq!(align_of::<ffi::ghostty_string_s>(), 8);
            assert_eq!(size_of::<ffi::ghostty_surface_config_s>(), 96);
            assert_eq!(align_of::<ffi::ghostty_surface_config_s>(), 8);
            assert_eq!(size_of::<ffi::ghostty_surface_size_s>(), 20);
            assert_eq!(align_of::<ffi::ghostty_surface_size_s>(), 4);
        }
    }

    #[test]
    fn runtime_config_struct_preserves_callback_pointer_layout_on_64_bit_targets() {
        #[cfg(target_pointer_width = "64")]
        {
            assert_eq!(size_of::<ffi::ghostty_target_s>(), 16);
            assert_eq!(align_of::<ffi::ghostty_target_s>(), 8);
            assert_eq!(size_of::<ffi::ghostty_input_trigger_s>(), 12);
            assert_eq!(align_of::<ffi::ghostty_input_trigger_s>(), 4);
            assert_eq!(size_of::<ffi::ghostty_action_key_sequence_s>(), 16);
            assert_eq!(align_of::<ffi::ghostty_action_key_sequence_s>(), 4);
            assert_eq!(size_of::<ffi::ghostty_action_key_table_s>(), 24);
            assert_eq!(align_of::<ffi::ghostty_action_key_table_s>(), 8);
            assert_eq!(size_of::<ffi::ghostty_action_u>(), 24);
            assert_eq!(align_of::<ffi::ghostty_action_u>(), 8);
            assert_eq!(size_of::<ffi::ghostty_action_s>(), 32);
            assert_eq!(align_of::<ffi::ghostty_action_s>(), 8);
            assert_eq!(size_of::<ffi::ghostty_runtime_config_s>(), 64);
            assert_eq!(align_of::<ffi::ghostty_runtime_config_s>(), 8);
        }
    }

    #[test]
    fn input_abi_structs_preserve_header_layout_on_64_bit_targets() {
        assert_eq!(size_of::<ffi::ghostty_input_action_e>(), size_of::<c_int>());
        assert_eq!(size_of::<ffi::ghostty_input_mods_e>(), size_of::<c_int>());
        assert_eq!(
            size_of::<ffi::ghostty_binding_flags_e>(),
            size_of::<c_int>()
        );
        assert_eq!(
            size_of::<ffi::ghostty_input_mouse_state_e>(),
            size_of::<c_int>()
        );
        assert_eq!(
            size_of::<ffi::ghostty_input_mouse_button_e>(),
            size_of::<c_int>()
        );
        assert_eq!(
            size_of::<ffi::ghostty_input_scroll_mods_t>(),
            size_of::<c_int>()
        );

        #[cfg(target_pointer_width = "64")]
        {
            assert_eq!(size_of::<ffi::ghostty_input_key_s>(), 32);
            assert_eq!(align_of::<ffi::ghostty_input_key_s>(), 8);
        }
    }

    #[test]
    fn runtime_and_surface_function_signatures_match_header_use_sites() {
        let _string_free: unsafe extern "C" fn(ffi::ghostty_string_s) = ffi::ghostty_string_free;
        let _app_focus: unsafe extern "C" fn(ffi::ghostty_app_t, bool) = ffi::ghostty_app_set_focus;
        let _surface_scale: unsafe extern "C" fn(ffi::ghostty_surface_t, f64, f64) =
            ffi::ghostty_surface_set_content_scale;
        let _surface_size: unsafe extern "C" fn(
            ffi::ghostty_surface_t,
        ) -> ffi::ghostty_surface_size_s = ffi::ghostty_surface_size;
        let _surface_process_exited: unsafe extern "C" fn(ffi::ghostty_surface_t) -> bool =
            ffi::ghostty_surface_process_exited;
        let _surface_needs_confirm_quit: unsafe extern "C" fn(ffi::ghostty_surface_t) -> bool =
            ffi::ghostty_surface_needs_confirm_quit;
        let _surface_foreground_pid: unsafe extern "C" fn(ffi::ghostty_surface_t) -> u64 =
            ffi::ghostty_surface_foreground_pid;
        let _surface_tty_name: unsafe extern "C" fn(
            ffi::ghostty_surface_t,
        ) -> ffi::ghostty_string_s = ffi::ghostty_surface_tty_name;
        let _surface_request_close: unsafe extern "C" fn(ffi::ghostty_surface_t) =
            ffi::ghostty_surface_request_close;
        let _surface_complete_clipboard_request: unsafe extern "C" fn(
            ffi::ghostty_surface_t,
            *const c_char,
            *mut c_void,
            bool,
        ) = ffi::ghostty_surface_complete_clipboard_request;
        let _surface_key_translation_mods: unsafe extern "C" fn(
            ffi::ghostty_surface_t,
            ffi::ghostty_input_mods_e,
        )
            -> ffi::ghostty_input_mods_e = ffi::ghostty_surface_key_translation_mods;
        let _surface_key: unsafe extern "C" fn(
            ffi::ghostty_surface_t,
            ffi::ghostty_input_key_s,
        ) -> bool = ffi::ghostty_surface_key;
        let _surface_key_is_binding: unsafe extern "C" fn(
            ffi::ghostty_surface_t,
            ffi::ghostty_input_key_s,
            *mut ffi::ghostty_binding_flags_e,
        ) -> bool = ffi::ghostty_surface_key_is_binding;
        let _surface_text: unsafe extern "C" fn(ffi::ghostty_surface_t, *const c_char, usize) =
            ffi::ghostty_surface_text;
        let _surface_preedit: unsafe extern "C" fn(ffi::ghostty_surface_t, *const c_char, usize) =
            ffi::ghostty_surface_preedit;
        let _surface_mouse_captured: unsafe extern "C" fn(ffi::ghostty_surface_t) -> bool =
            ffi::ghostty_surface_mouse_captured;
        let _surface_mouse_button: unsafe extern "C" fn(
            ffi::ghostty_surface_t,
            ffi::ghostty_input_mouse_state_e,
            ffi::ghostty_input_mouse_button_e,
            ffi::ghostty_input_mods_e,
        ) -> bool = ffi::ghostty_surface_mouse_button;
        let _surface_mouse_pos: unsafe extern "C" fn(
            ffi::ghostty_surface_t,
            f64,
            f64,
            ffi::ghostty_input_mods_e,
        ) = ffi::ghostty_surface_mouse_pos;
        let _surface_mouse_scroll: unsafe extern "C" fn(
            ffi::ghostty_surface_t,
            f64,
            f64,
            ffi::ghostty_input_scroll_mods_t,
        ) = ffi::ghostty_surface_mouse_scroll;
        let _surface_mouse_pressure: unsafe extern "C" fn(ffi::ghostty_surface_t, u32, f64) =
            ffi::ghostty_surface_mouse_pressure;
        let _surface_ime_point: unsafe extern "C" fn(
            ffi::ghostty_surface_t,
            *mut f64,
            *mut f64,
            *mut f64,
