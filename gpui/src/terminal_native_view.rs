...
#[cfg(target_os = "macos")]
impl AppOwnedTerminalHostNativeView {
...
mod tests {
    ...
    #[cfg(target_os = "macos")]
    use crate::terminal_surface_host::{
        NativeTerminalSurfaceAttachmentPlan, NativeTerminalSurfaceHost,
        NativeTerminalSurfaceHostCommand,
    };
    use crate::terminal_surface_host::{
        NativeTerminalSurfaceHostId, NativeTerminalSurfacePlatformCommandPayload,
    };
    #[cfg(target_os = "macos")]
    use crate::terminal_surface_lifecycle::{
        NativeTerminalSurfaceLifecycleDecision, NativeTerminalSurfaceLifecycleState,
    ...
    fn platform_bounds() -> NativeTerminalSurfacePlatformBounds {
    ...
    #[cfg(target_os = "macos")]
    fn first_plan_from_host_commands(
    ...
    #[cfg(target_os = "macos")]
    fn test_parent_view_handle() -> TerminalHostParentNativeViewHandle {
        let pointer = NonNull::new(0x2000usize as *mut c_void).unwrap();
        unsafe { TerminalHostParentNativeViewHandle::from_existing_parent_view(pointer) }
    }

    #[cfg(target_os = "macos")]
    fn test_native_view_pointer() -> NonNull<c_void> {
        NonNull::new(0x3000usize as *mut c_void).unwrap()
    }

    #[cfg(target_os = "macos")]
    fn test_real_native_view_handle() -> RealTerminalNativeViewHandle {
        unsafe {
            RealTerminalNativeViewHandle::from_existing_native_view(test_native_view_pointer())
        }
    }

    #[cfg(target_os = "macos")]
    fn borrowed_real_native_view_handle_for_test() -> RealTerminalNativeViewHandle {
        let pointer = NonNull::new(0x4000usize as *mut c_void).unwrap();
        unsafe { RealTerminalNativeViewHandle::from_existing_native_view(pointer) }
    }

    #[cfg(target_os = "macos")]
    fn owned_host_view_for_test(
        destroy_count: Arc<AtomicUsize>,
        destroyed_pointer: Arc<AtomicUsize>,
    ) -> OwnedTerminalHostNativeView {
        unsafe {
            OwnedTerminalHostNativeView::from_created_native_view_for_test(
                test_native_view_pointer(),
                destroy_count,
                destroyed_pointer,
            )
        }
    ...
    #[cfg(target_os = "macos")]
    fn mounted_owned_host_view_for_test(
        destroy_count: Arc<AtomicUsize>,
        destroyed_pointer: Arc<AtomicUsize>,
    ) -> (
        NativeTerminalSurfaceHost,
        NativeTerminalSurfaceLifecycleState,
        HashMap<AgentsTerminalBodyMountSlotId, AppOwnedTerminalHostNativeView>,
        NativeTerminalSurfaceAttachmentPlan,
    ) {
        let slot_id = test_slot();
        ...
        let plan = first_plan_from_host_commands(&commands);
        let mut lifecycle = NativeTerminalSurfaceLifecycleState::new();
        let decisions = lifecycle.reconcile_host_commands(&commands);
        let mut owned_host_views = HashMap::new();

        let frame_operations = reconcile_app_owned_terminal_host_native_view(
            &mut owned_host_views,
            &mut lifecycle,
            test_parent_view_handle().as_ptr(),
            &commands,
            &decisions,
            |_request| {
                Ok(owned_host_view_for_test(
                    destroy_count.clone(),
        ...
        assert!(frame_operations.is_empty());
        assert!(owned_host_views.contains_key(&slot_id));
        (host, lifecycle, owned_host_views, plan)
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn ghostty_config_request_for_app_owned_host_view_preserves_nsview_and_scale() {
        let destroy_count = Arc::new(AtomicUsize::new(0));
        let destroyed_pointer = Arc::new(AtomicUsize::new(0));
        let (_host, _lifecycle, owned_host_views, _plan) =
            mounted_owned_host_view_for_test(destroy_count, destroyed_pointer);

        let request = ghostty_surface_config_request_for_app_owned_terminal_host_native_view(
            owned_host_views.get(&test_slot()),
            2.25,
        )
        .unwrap()
        .expect("owned host view should produce a config request");
        let config = request.to_ffi_config();

        assert_eq!(
            unsafe { config.platform.macos }.nsview,
            test_native_view_pointer().as_ptr()
        );
        assert_eq!(config.userdata, test_native_view_pointer().as_ptr());
        assert_eq!(config.scale_factor, 2.25);
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn ghostty_config_request_is_absent_without_app_owned_host_view() {
        let request = ghostty_surface_config_request_for_app_owned_terminal_host_native_view::<
            AgentsTerminalBodyMountSlotId,
        >(None, 2.0)
        .unwrap();
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn ghostty_config_request_rejects_invalid_scale_for_app_owned_host_view() {
        let destroy_count = Arc::new(AtomicUsize::new(0));
        let destroyed_pointer = Arc::new(AtomicUsize::new(0));
        let (_host, _lifecycle, owned_host_views, _plan) =
            mounted_owned_host_view_for_test(destroy_count, destroyed_pointer);

        let error = ghostty_surface_config_request_for_app_owned_terminal_host_native_view(
            owned_host_views.get(&test_slot()),
            0.0,
        )
        .expect_err("invalid scale must not produce a config request");

        assert!(matches!(
            error,
            GhosttySurfaceConfigRequestError::InvalidScaleFactor(0.0)
        ));
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn app_owned_host_focus_identity_targets_exact_slot_and_real_view() {
        let destroy_count = Arc::new(AtomicUsize::new(0));
        let destroyed_pointer = Arc::new(AtomicUsize::new(0));
        let (_host, _lifecycle, owned_host_views, _plan) =
            mounted_owned_host_view_for_test(destroy_count, destroyed_pointer);
        ...
        let focus_identity =
            app_owned_terminal_host_focus_identity(owned_host_views.get(&test_slot()))
                .expect("owned host view should produce a focus identity");

        assert!(focus_identity.slot_id() == test_slot());
        assert!(focus_identity.real_view == test_real_native_view_handle());
        assert!(
            app_owned_terminal_host_focus_identity::<AgentsTerminalBodyMountSlotId>(None).is_none()
        );
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn app_owned_host_focus_handoff_is_idempotent_unless_forced_or_identity_changes() {
        let destroy_count = Arc::new(AtomicUsize::new(0));
        let destroyed_pointer = Arc::new(AtomicUsize::new(0));
        let (_host, _lifecycle, owned_host_views, _plan) =
            mounted_owned_host_view_for_test(destroy_count, destroyed_pointer);
        let focus_identity =
            app_owned_terminal_host_focus_identity(owned_host_views.get(&test_slot()))
                .expect("owned host view should produce a focus identity");
        let changed_identity = AppOwnedTerminalHostFocusIdentity {
            slot_id: AgentsTerminalBodyMountSlotId {
                pane_id: WorkspacePaneId(20),
                session_id: TerminalSessionId(201),
            },
            real_view: borrowed_real_native_view_handle_for_test(),
        };

        assert!(app_owned_terminal_host_focus_should_execute(
            None,
            Some(focus_identity),
            false,
        ));
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn app_owned_host_view_creation_uses_same_plan_request_and_marks_lifecycle_ready() {
        let slot_id = test_slot();
        let bounds = test_bounds(12.0, 24.0, 640.0, 360.0);
        let mut host = NativeTerminalSurfaceHost::new();
        let commands =
        ...
        let destroy_count = Arc::new(AtomicUsize::new(0));
        let destroyed_pointer = Arc::new(AtomicUsize::new(0));
        let parent = test_parent_view_handle();
        ...
        assert!(frame_operations.is_empty());
        let request = captured_request.expect("creation should build a host-view request");
        assert!(request.parent_view().as_ptr() == parent.as_ptr());
        assert!(request.bounds() == NativeTerminalSurfacePlatformBounds::from_gpui_bounds(bounds));
        assert!(
            owned_host_views
                .get(&slot_id)
                .is_some_and(|owned| owned.plan() == plan)
        );
        let ready_decisions =
            lifecycle.reconcile_host_commands(&[NativeTerminalSurfaceHostCommand::NoOp { plan }]);
        assert!(matches!(
            ready_decisions.as_slice(),
            [NativeTerminalSurfaceLifecycleDecision::NoOp { plan: ready_plan }]
                if *ready_plan == plan
        ));
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn attach_or_show_ready_decision_produces_no_hidden_host_frame_operation() {
        let destroy_count = Arc::new(AtomicUsize::new(0));
        let destroyed_pointer = Arc::new(AtomicUsize::new(0));
        let (_host, _lifecycle, owned_host_views, plan) =
            mounted_owned_host_view_for_test(destroy_count, destroyed_pointer);
        let decisions = [
            NativeTerminalSurfaceLifecycleDecision::CanExecuteWithRealView {
                command: NativeTerminalSurfaceHostCommand::AttachOrShow { plan }
                    .to_platform_command(),
                real_view: test_real_native_view_handle(),
            },
        ];

        let frame_operations = frame_only_operations_for_app_owned_terminal_host_native_view(
            &owned_host_views,
            &decisions,
        );
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn hidden_or_no_current_detach_drops_app_owned_host_view() {
        for (agents_visible, current_slots) in [(false, vec![test_slot()]), (true, Vec::new())] {
            let destroy_count = Arc::new(AtomicUsize::new(0));
            let destroyed_pointer = Arc::new(AtomicUsize::new(0));
            let (mut host, mut lifecycle, mut owned_host_views, _plan) =
                mounted_owned_host_view_for_test(destroy_count.clone(), destroyed_pointer.clone());
            let commands =
                host.sync_visible_agents_slots(agents_visible, &current_slots, &HashMap::new());
            let decisions = lifecycle.reconcile_host_commands(&commands);
            ...
            let frame_operations = reconcile_app_owned_terminal_host_native_view(
                ...
                &commands,
                &decisions,
                |_request| panic!("detach must not create a replacement host view"),
            );

            assert!(frame_operations.is_empty());
            assert!(owned_host_views.is_empty());
            assert_eq!(destroy_count.load(Ordering::SeqCst), 1);
            assert_eq!(
                destroyed_pointer.load(Ordering::SeqCst),
                test_native_view_pointer().as_ptr() as usize
            );
        }
    }
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn pre_layout_empty_decision_batch_preserves_app_owned_host_view() {
        let destroy_count = Arc::new(AtomicUsize::new(0));
        let destroyed_pointer = Arc::new(AtomicUsize::new(0));
        let (mut host, mut lifecycle, mut owned_host_views, plan) =
            mounted_owned_host_view_for_test(destroy_count.clone(), destroyed_pointer.clone());
        let reset_commands = host.sync_visible_agents_slots(true, &[test_slot()], &HashMap::new());
        ...
        let frame_operations = reconcile_app_owned_terminal_host_native_view(
            ...
            &reset_commands,
            &reset_decisions,
            |_request| panic!("pre-layout empty sync must not create a new host view"),
        );
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn move_resize_updates_app_owned_plan_without_recreating_hidden_host_view() {
        let destroy_count = Arc::new(AtomicUsize::new(0));
        let destroyed_pointer = Arc::new(AtomicUsize::new(0));
        let (mut host, mut lifecycle, mut owned_host_views, first_plan) =
            mounted_owned_host_view_for_test(destroy_count.clone(), destroyed_pointer.clone());
        let moved_bounds = test_bounds(18.0, 30.0, 720.0, 390.0);
        let move_commands = host.sync_visible_agents_slots(
            true,
            &[test_slot()],
            &bounds_map(test_slot(), moved_bounds),
        );
        let decisions = lifecycle.reconcile_host_commands(&move_commands);
        let moved_plan = first_plan_from_host_commands(&move_commands);
        ...
        let frame_operations = reconcile_app_owned_terminal_host_native_view(
            ...
            &move_commands,
            &decisions,
            |_request| panic!("same host move/resize must not recreate the hidden host view"),
        );

        assert!(moved_plan.same_attachment_identity(first_plan));
        assert!(moved_plan != first_plan);
        assert!(matches!(
            decisions.as_slice(),
            [NativeTerminalSurfaceLifecycleDecision::CanExecuteWithRealView { .. }]
        ));
        assert!(
            frame_operations
                == vec![AppOwnedTerminalHostFrameOperation::set_frame(
                    test_real_native_view_handle(),
                    NativeTerminalSurfacePlatformBounds::from_gpui_bounds(moved_bounds),
                )]
        );
        assert!(
            frame_operations[0].platform_operation()
                == NativeTerminalSurfacePlatformOperation::SetFrame {
                    bounds: NativeTerminalSurfacePlatformBounds::from_gpui_bounds(moved_bounds),
                }
        );
        assert!(
            owned_host_views
                .get(&test_slot())
                .is_some_and(|owned| owned.plan() == moved_plan)
        );
        assert_eq!(destroy_count.load(Ordering::SeqCst), 0);
    }
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn non_matching_ready_view_decisions_do_not_operate_on_owned_hidden_host() {
        let destroy_count = Arc::new(AtomicUsize::new(0));
        let destroyed_pointer = Arc::new(AtomicUsize::new(0));
        let (_host, _lifecycle, owned_host_views, plan) =
            mounted_owned_host_view_for_test(destroy_count.clone(), destroyed_pointer.clone());
        let other_slot = AgentsTerminalBodyMountSlotId {
            pane_id: WorkspacePaneId(20),
            session_id: TerminalSessionId(201),
        };
        let other_plan = NativeTerminalSurfaceAttachmentPlan {
            host_id: NativeTerminalSurfaceHostId::from_slot_id(other_slot),
            slot_id: other_slot,
            bounds: test_bounds(700.0, 40.0, 500.0, 260.0),
        };
        let decisions = [
            NativeTerminalSurfaceLifecycleDecision::CanExecuteWithRealView {
                command: NativeTerminalSurfaceHostCommand::MoveOrResize { plan }
                    .to_platform_command(),
                real_view: borrowed_real_native_view_handle_for_test(),
            },
            NativeTerminalSurfaceLifecycleDecision::CanExecuteWithRealView {
                command: NativeTerminalSurfaceHostCommand::MoveOrResize { plan: other_plan }
                    .to_platform_command(),
                real_view: test_real_native_view_handle(),
            },
        ];
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn null_parent_or_create_failure_leaves_lifecycle_awaiting_without_owner() {
        let slot_id = test_slot();
        let bounds = test_bounds(12.0, 24.0, 640.0, 360.0);
        let mut host = NativeTerminalSurfaceHost::new();
        let commands =
        ...
        let mut null_parent_lifecycle = NativeTerminalSurfaceLifecycleState::new();
        let null_parent_decisions = null_parent_lifecycle.reconcile_host_commands(&commands);
        let mut null_parent_owned_host_views = HashMap::new();
        let mut create_attempts = 0usize;
        let null_parent_frame_operations = reconcile_app_owned_terminal_host_native_view(
            &mut null_parent_owned_host_views,
            &mut null_parent_lifecycle,
            std::ptr::null_mut(),
            &commands,
            &null_parent_decisions,
            |_request| {
                create_attempts += 1;
                unreachable!("null parent must fail before the create boundary")
            },
        );

        assert!(null_parent_owned_host_views.is_empty());
        assert!(null_parent_frame_operations.is_empty());
        assert_eq!(create_attempts, 0);
        assert!(matches!(
            null_parent_lifecycle
                .reconcile_host_commands(&[NativeTerminalSurfaceHostCommand::NoOp { plan }])
                .as_slice(),
            [NativeTerminalSurfaceLifecycleDecision::NeedsRealNativeView { plan: awaiting_plan }]
                if *awaiting_plan == plan
        ));

        let mut create_failure_lifecycle = NativeTerminalSurfaceLifecycleState::new();
        let create_failure_decisions = create_failure_lifecycle.reconcile_host_commands(&commands);
        let mut create_failure_owned_host_views = HashMap::new();
        let mut failed_create_attempts = 0usize;
        let create_failure_frame_operations = reconcile_app_owned_terminal_host_native_view(
            &mut create_failure_owned_host_views,
            &mut create_failure_lifecycle,
            test_parent_view_handle().as_ptr(),
            &commands,
            &create_failure_decisions,
            |_request| {
                failed_create_attempts += 1;
                Err(TerminalHostNativeViewCreateError::CreateReturnedNull)
            },
        );

        assert!(create_failure_owned_host_views.is_empty());
        assert!(create_failure_frame_operations.is_empty());
        assert_eq!(failed_create_attempts, 1);
        assert!(matches!(
            create_failure_lifecycle
                .reconcile_host_commands(&[NativeTerminalSurfaceHostCommand::NoOp { plan }])
                .as_slice(),
            [NativeTerminalSurfaceLifecycleDecision::NeedsRealNativeView { plan: awaiting_plan }]
                if *awaiting_plan == plan
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn host_view_create_request_requires_non_null_parent_and_valid_bounds() {
        let parent = unsafe {
            TerminalHostParentNativeViewHandle::try_from_raw(test_parent_view_handle().as_ptr())
                .unwrap()
        };
        let bounds = NativeTerminalSurfacePlatformBounds {
            x: 0.0,
            y: 24.5,
            width: 0.0,
            height: 359.625,
        ...
        let request = TerminalHostNativeViewCreateRequest::try_new(parent, bounds).unwrap();

        assert!(request.parent_view() == parent);
        assert!(request.bounds() == bounds);
        assert!(matches!(
            unsafe { TerminalHostParentNativeViewHandle::try_from_raw(std::ptr::null_mut()) },
            Err(TerminalHostNativeViewCreateError::NullParentNativeView)
        ));
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn host_view_create_request_rejects_non_finite_or_negative_bounds() {
        let parent = test_parent_view_handle();
        let base = NativeTerminalSurfacePlatformBounds {
            x: 12.25,
            y: 24.5,
            width: 640.5,
            height: 359.625,
        ...
        let invalid_cases = [
            (
                TerminalHostNativeViewBoundsField::X,
                NativeTerminalSurfacePlatformBounds {
                    x: f64::NAN,
                    ..base
                },
            ...
            (
                TerminalHostNativeViewBoundsField::Y,
                NativeTerminalSurfacePlatformBounds { y: -1.0, ..base },
            ),
            (
                TerminalHostNativeViewBoundsField::Width,
                NativeTerminalSurfacePlatformBounds {
                    width: f64::INFINITY,
                    ..base
                },
            ...
            (
                TerminalHostNativeViewBoundsField::Height,
                NativeTerminalSurfacePlatformBounds {
                    height: -0.5,
                    ..base
                },
        ...
        for (expected_field, bounds) in invalid_cases {
            let error = TerminalHostNativeViewCreateRequest::try_new(parent, bounds)
                .expect_err("invalid bounds should not produce a create request");

            assert!(matches!(
                error,
                TerminalHostNativeViewCreateError::InvalidBounds { field, value }
                    if field == expected_field && (value.is_nan() || value < 0.0 || value.is_infinite())
            ));
        }
    }
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn owned_host_view_destroy_is_separate_from_borrowed_handles() {
        let destroy_count = Arc::new(AtomicUsize::new(0));
        let destroyed_pointer = Arc::new(AtomicUsize::new(0));
        let borrowed = unsafe {
            RealTerminalNativeViewHandle::from_existing_native_view(test_native_view_pointer())
        ...
        {
            let _borrowed_non_owner = borrowed;
        }
        ...
        assert_eq!(destroy_count.load(Ordering::SeqCst), 0);
        {
            ...
            assert!(owned.real_native_view_handle() == borrowed);
            assert_eq!(destroy_count.load(Ordering::SeqCst), 0);
        }
    ...
    #[test]
    fn attach_or_show_maps_to_frame_then_show_without_focus() {
        let operations = operations_for_native_terminal_platform_command(
            NativeTerminalSurfacePlatformCommand::AttachOrShow { payload: payload() },
        );
    ...
    #[test]
    fn move_or_resize_maps_to_frame_only_without_focus() {
        let operations = operations_for_native_terminal_platform_command(
            NativeTerminalSurfacePlatformCommand::MoveOrResize { payload: payload() },
        );
    ...
    #[test]
    fn hide_and_detach_maps_to_hide_only() {
        let operations = operations_for_native_terminal_platform_command(
            NativeTerminalSurfacePlatformCommand::HideAndDetach { payload: payload() },
        );

        assert!(operations == vec![NativeTerminalSurfacePlatformOperation::Hide]);
    }

    #[test]
    fn no_op_maps_to_no_operations() {
        let operations = operations_for_native_terminal_platform_command(
            NativeTerminalSurfacePlatformCommand::NoOp { payload: payload() },
        );
    ...
    #[test]
    fn generic_host_commands_never_map_to_focus_operation() {
        for command in [
            NativeTerminalSurfacePlatformCommand::AttachOrShow { payload: payload() },
            NativeTerminalSurfacePlatformCommand::MoveOrResize { payload: payload() },
            NativeTerminalSurfacePlatformCommand::HideAndDetach { payload: payload() },
            ...
            assert!(
                !operations_for_native_terminal_platform_command(command)
                    .contains(&NativeTerminalSurfacePlatformOperation::Focus)
            );
        }
    }
}
