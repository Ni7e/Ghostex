...
import { TOOLTIP_DELAY_MS, TooltipProvider } from '@/packages/components/ui/tooltip';
import {
...
import { type ConversationActionState, type TicketContextMenuState } from './types';
import { PROJECT_BOARD_MAX_VISIBLE_TICKETS_PER_COLUMN } from './constants';
import { BOARD_CARD_VIEW_DEFAULTS, type BoardCardViewOptions } from './card-view-options';
import { getPrimaryUsableConversationLink, ConversationLinkName } from './ticket-detail';
 ...
 */
const CHIP_TONE_COLORS = ['#5ea4ff', '#e7b85b', '#b18cff', '#6fd19c', '#f28b8b', '#95d7f6', '#e79ad0'];
...
function TicketPriorityIcon({ priority }: { priority: number | undefined }) {
  ...
  const tone = value === 1 ? 'text-amber-400/90' : value === 2 ? 'text-sky-400/80' : 'text-muted-foreground/70';
  return (
...
export function TicketCard({
  ...
  const showChips =
    ...
    (cardView.showDetails && (Boolean(tshirt) || blockedByCount > 0 || blockingCount > 0 || commentCount > 0));
  ...
  return (
    ...
    >
        ...
        <div className='flex items-center justify-between gap-2 text-xs font-normal text-muted-foreground [&_svg]:size-3.5'>
          <TooltipProvider delayDuration={TOOLTIP_DELAY_MS}>
            <span className='flex min-w-0 items-center gap-1.5'>
              ...
              <ConversationLinkName className='truncate' label={primaryLinkLabel} />
              {additionalLinkCount > 0 ? (
                <span className='shrink-0 text-muted-foreground/70'>+{additionalLinkCount}</span>
              ) : null}
            </span>
          </TooltipProvider>
          <Button
            aria-label={
              primaryLinkActionKind === 'resume' ? 'Resume linked conversation' : 'Jump to linked conversation'
            }
      ...
      {cardView.showDetails && creator ? (
        <div className='truncate text-[11px] font-normal text-muted-foreground/60'>Created by {creator}</div>
      ) : null}
    </Card>
  );
}
...
export function ProjectBoardTicketContextMenu({
  ...
  useLayoutEffect(() => {
    ...
    const left = Math.max(
      ...
      Math.min(position.x, window.innerWidth - bounds.width - PROJECT_BOARD_CONTEXT_MENU_VIEWPORT_MARGIN_PX)
    );
    const top = Math.max(
      PROJECT_BOARD_CONTEXT_MENU_VIEWPORT_MARGIN_PX,
      Math.min(position.y, window.innerHeight - bounds.height - PROJECT_BOARD_CONTEXT_MENU_VIEWPORT_MARGIN_PX)
    );
    setMenuStyle({
