...
) -> Result<GpuiSidebarWorkspaceTerminalFocusMessage, GpuiGxserverPresentationFocusStateContractError>
{
    ...
    let keep_sleeping = match object.get("keepSleeping") {
        None => false,
        Some(value) => value
            .as_bool()
            .ok_or(GpuiGxserverPresentationFocusStateContractError::MalformedJson)?,
...
pub(crate) fn gpui_sidebar_session_completion_sound_from_json(
    text: &str,
) -> Result<(String, Option<String>), GpuiGxserverPresentationFocusStateContractError> {
    let value = serde_json::from_str::<serde_json::Value>(text)
        .map_err(|_| GpuiGxserverPresentationFocusStateContractError::MalformedJson)?;
    let object = gpui_gxserver_focus_contract_object(&value)?;
    reject_unexpected_gxserver_focus_contract_keys(
        object,
        &["version", "type", "sound", "sessionId"],
    )?;
    ...
    let session_id = match object.get("sessionId") {
        ...
        Some(value) => {
            let value = value
                ...
                .ok_or(GpuiGxserverPresentationFocusStateContractError::MalformedField)?
                .trim();
            // A SIDEBAR session id, which is the scoped form for a remote machine's row
            // (`combined-session:<p>:<s>` or `remote:<m>:session:<p>:<s>`), so the per-daemon
            // allowlists do not apply. It is only ever a map key compared with ids the renderer
            // built itself, and it reaches no daemon, path or command; bounded and free of control
            // characters is the whole contract.
            if value.is_empty()
    ...
    Ok((trimmed.to_string(), session_id))
}
