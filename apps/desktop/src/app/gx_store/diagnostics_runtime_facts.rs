//! The periodic line of the runtime facts channel, in a sibling because `diagnostics.rs` is over
//! the size ceiling and waiting for a quiet window.
//!
...
use super::diagnostics::{GxStoreDiagnostics, record, routine_logging_enabled};
...
impl GxStoreDiagnostics {
    ...
    pub(super) fn runtime_facts_summary(&mut self, counters: RuntimeFactsCounters) {
        if self.runtime_facts_summary_written == Some(counters)
            || self
        ...
        self.runtime_facts_summary_written = Some(counters);
        record(
