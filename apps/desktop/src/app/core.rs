...
pub struct GhostexGpuiApp {
    ...
    */
    ...
    session-to-renderer binding. Keyed by shell TerminalSessionId because
    the binding follows a tab across pane drags. Hidden idle pages pass a
    fresh release probe before their renderer can serve another session.
    Local chat surfaces
    ...
    */
    ...
    /// Sessions that have rendered the terminal agent action bar at least
    /// once. While a just-selected session's bar data lags a frame, its bar is
    /// reserved at full height instead of vanishing, so the terminal body's
    /// row count — and with it the whole claim-dump-redraw pipeline — does not
    /// churn on every session switch. Runtime-only.
