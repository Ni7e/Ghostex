...
impl GhostexGpuiApp {
    pub(crate) fn render_command_pane_leaf(
        ...
        let in_view = self.command_pane.dock_for_group(group_id) == Some(CommandPaneDock::View);
    ...
    pub(crate) fn render_command_pane_titlebar(
        ...
        let in_view = self.command_pane.dock_for_group(group_id) == Some(CommandPaneDock::View);
        let keep_open_visible = !in_view && self.command_pane_keep_open_control_visible(true);
        let show_tab_add_button = if in_view {
            CommandPaneModel::view_leaf_tab_add_visible_for_chrome_width(chrome_width)
        } else {
            command_pane_inline_tab_add_visible_for_chrome_width(
                chrome_width,
                true,
                keep_open_visible,
            )
        ...
        let sticky_trailing_inset = if in_view {
            CommandPaneModel::view_leaf_sticky_active_tab_trailing_inset(show_tab_add_button)
        } else {
            command_pane_sticky_active_tab_trailing_inset(
                true,
                show_tab_add_button,
                keep_open_visible,
            )
    ...
    pub(crate) fn render_command_pane_strip(
        ...
        let flat_tabs = self.command_pane.panel_flat_tab_ids();
        ...
        let wheel_scroll_handle = scroll_handle.clone();
        let active_flat_tab = self
            .command_pane
            .active_group_and_session_id_in_dock(CommandPaneDock::Panel)
            .and_then(|(active_group_id, active_session_id)| {
                flat_tabs
                    .iter()
