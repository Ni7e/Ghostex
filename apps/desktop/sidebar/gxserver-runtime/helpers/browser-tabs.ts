...
export function normalizeGpuiBrowserTabs(tabs: readonly GpuiBrowserTabSummary[] | unknown): GpuiBrowserTabSummary[] {
  ...
  return tabs.slice(0, 256).flatMap((tab) => {
    ...
    const title = typeof record.title === 'string' ? normalizeNonEmptyString(record.title)?.slice(0, 512) : undefined;
    const url =
      typeof record.url === 'string' ? record.url.trim().slice(0, GPUI_SIDEBAR_OPEN_BROWSER_URL_MAX_CHARS) : '';
...
export function normalizeGpuiBrowserFaviconUrl(value: unknown): string | undefined {
  ...
  try {
    ...
    return normalized.length <= GPUI_SIDEBAR_BROWSER_FAVICON_URL_MAX_CHARS ? normalized : undefined;
  } catch {
    return undefined;
  }
}

export function relayoutGpuiSidebarSessions(sessions: readonly SidebarSessionItem[]): SidebarSessionItem[] {
