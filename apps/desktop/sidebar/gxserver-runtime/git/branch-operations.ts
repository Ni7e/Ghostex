...
export const gpuiSidebarRuntimeGitBranchOperationsMethods = {
  ...
  async commitWithMessage(
    ...
    if (parsedMessage.subject) {
      ...
      resolvedMessage = await this.generateCommitMessage(
        project,
        filePaths,
        options.agentId,
      );
    ...
    if (options.commitOnNewRef) {
      await this.checkoutSidebarGitFeatureBranch(
        project,
    ...
    const commitResult = await this.runGitAction(project, {
      ...
      noVerify:
        await this.shouldBypassMissingBeadsDatabasePreCommitHook(project),
    });
  ...
  async generateCommitMessage(
    ...
    if (!filePaths || filePaths.length === 0) {
      throw new Error(
        "Select at least one changed file before generating a commit message.",
      );
    }
    ...
    return this.client.rpc<GxserverGenerateCommitMessageResult>(
  ...
  async pushCurrentBranch(
    ...
    gitState: Pick<
      SidebarGitState,
      "branch" | "behindCount" | "hasOriginRemote" | "hasUpstream"
    >,
  ): Promise<void> {
    ...
    const push = gitState.hasUpstream
      ...
      : gitState.hasOriginRemote
        ? await this.runGitAction(project, {
  ...
  async syncCurrentBranchWithRemote(
    ...
    if (gitState.hasUpstream) {
      const pull = await this.runGitAction(project, {
  ...
  async commitRemoteWithMessage(
      ...
      if (addResult.exitCode !== 0) {
        throw new Error(
          addResult.stderr.trim() ||
            ...
            "Could not stage remote changes.",
        );
      }
    } else {
    ...
    if (options.commitOnNewRef) {
      await this.checkoutRemoteSidebarGitFeatureBranch(
        remoteScope,
        resolvedMessage.subject,
      );
    }
    const commitResult = await this.runRemoteGitAction(remoteScope, {
      ...
      messageSubject: resolvedMessage.subject,
      noVerify:
        await this.shouldBypassRemoteMissingBeadsDatabasePreCommitHook(
          remoteScope,
        ),
    });
    if (commitResult.exitCode !== 0) {
      throw new Error(
        commitResult.stderr.trim() ||
          ...
          "Could not commit remote changes.",
      );
    }
  },
  ...
  async checkoutRemoteSidebarGitFeatureBranch(
    ...
    const result =
      await this.requestRemoteGxserver<GxserverCheckoutProjectNewBranchResult>(
        remoteScope.machineId,
  ...
  async pushRemoteCurrentBranch(
    ...
    const push = gitState.hasUpstream
      ...
      : gitState.hasOriginRemote
        ? await this.runRemoteGitAction(remoteScope, {
            action: "pushSetUpstreamCurrent",
          })
        : undefined;
    if (!push) {
      throw new Error('Add an "origin" remote before pushing.');
  ...
  async syncRemoteCurrentBranchWithRemote(
    ...
    if (gitState.hasUpstream) {
      const pull = await this.runRemoteGitAction(remoteScope, {
        action: "pullFastForward",
      });
      if (pull.exitCode !== 0) {
        throw new Error(
  ...
  async shouldBypassRemoteMissingBeadsDatabasePreCommitHook(
    ...
    const beadsStorage = await this.runRemoteBeadsAction(remoteScope, {
    ...
    try {
      const status = await this.runRemoteBeadsAction(remoteScope, {
        action: "status",
      });
      return (
        status.exitCode !== 0 &&
  ...
  async shouldBypassMissingBeadsDatabasePreCommitHook(
    ...
    const beadsStorage = await this.runBeadsAction(project, {
      action: "storageExists",
    });
    if (beadsStorage.exitCode !== 0 || beadsStorage.stdout.trim() !== "true") {
      return false;
