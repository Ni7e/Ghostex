...
import {
  ...
  SESSION_CHAT_CONTEXT_DETAILS_STORES,
...
import {
  ...
  type AnswerDrafts,
} from '@/packages/shared/session-chat-controller/question-drafts';
import {
...
} from '@/packages/core-ui/chat/session-chat-summary-override';
import {
  readStoredSessionChatVerbose,
  writeStoredSessionChatVerbose,
} from '@/packages/core-ui/chat/session-chat-verbose-override';
...
export async function nativeComposerRequest(sessionKey: string, request: NativeComposerRequest): Promise<unknown> {
  switch (request.operation) {
    case 'contextSave': {
      if (!request.agent || !(request.agent in SESSION_CHAT_CONTEXT_DETAILS_STORES) || !request.preferences)
        throw new Error('Context preferences require an agent and rows.');
      writeSessionChatContextDetailsPreferences(request.preferences, request.agent);
      await flushClientStorage([SESSION_CHAT_CONTEXT_DETAILS_STORES[request.agent]]);
      return true;
    }
    case 'optionWrite':
    case 'modelWrite':
    case 'modelAck': {
      ...
      if (!key || (key !== sessionKey && !key.startsWith(`${sessionKey}#`)))
        throw new Error('The option identity does not belong to this session.');
    ...
    case 'asyncQuestionRead':
      return asyncQuestionStorage(sessionKey).read();
    ...
    case 'questionClear': {
      ...
      const drafts =
        request.operation === 'questionClear'
          ? remainingQuestionDrafts(readQuestionDrafts(key), request.answers ?? {})
          : (request.answers ?? {});
    ...
    case 'history':
      return listSentSessionChatMessages()
        .map((message) => message.content)
        .reverse();
    case 'summary':
      writeStoredSessionChatSummary(sessionKey, request.enabled === true);
      return request.enabled === true;
    case 'verbose':
      writeStoredSessionChatVerbose(sessionKey, request.enabled === true);
      return request.enabled === true;
    case 'receive': {
      if (typeof request.text !== 'string' || typeof request.current !== 'string')
        throw new Error('A transfer requires the incoming and current drafts.');
      ...
      const entry =
        disposition === 'accept' ? writeStoredSessionChatDraft(sessionKey, request.text, undefined, version) : stored;
    ...
    case 'read': {
       ...
       */
      const version =
        stored?.version && !stored.submitted && !stored.parked ? stored.version : nextSessionChatDraftVersion();
      return {
        sessionKey,
        chatSettings: nativeChatSettings(sessionKey),
        contextPreferences: {
        ...
        optionStates: Object.fromEntries(
          storedSessionChatOptionKeys(sessionKey).map((key) => [key, readStoredSessionChatOptions(key)])
        ),
        modelOutboxes: Object.fromEntries(
          storedModelSelectionKeys(sessionKey).map((key) => [key, modelSelectionPersistence.read(key)])
        ),
        dismissedNotice: readStoredDismissedNotice(sessionKey),
        clientId: sessionChatDraftClientId(),
        entry: { ...stored, version },
        nextVersion: nextSessionChatDraftVersion(),
        summaryMode: readStoredSessionChatSummary(sessionKey),
        verboseOverride: readStoredSessionChatVerbose(sessionKey),
      };
    }
    case 'write': {
      if (typeof request.text !== 'string' || !request.version)
        throw new Error('A draft edit requires text and a revision.');
      const entry = writeStoredSessionChatDraft(
        sessionKey,
        request.text,
        undefined,
        request.version,
    ...
    case 'flush':
      ...
      return true;
    case 'submitted': {
      clearStoredSessionChatDraftIfUnchanged(sessionKey, {
        text: request.text ?? '',
        version: request.version,
        updatedAt: Date.now(),
      });
    ...
    case 'park': {
      ...
      if (
        ...
        entry.version?.draftId !== request.version?.draftId ||
        entry.version?.revision !== request.version?.revision
      )
    ...
    case 'deliveries':
      recordDeliveredSessionChatDrafts(request.deliveries ?? []);
      return true;
  }
}
