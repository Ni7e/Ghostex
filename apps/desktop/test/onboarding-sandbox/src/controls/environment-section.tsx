    ...
    <Section defaultOpen id='environment' title='Environment'>
      ...
      <SubGroup id='more-agents' title={`More agents (${otherAgentIds.length})`}>
        <div className='cp-btn-row'>
          ...
          <Btn onClick={() => setEveryAgent({ cliInstalled: false, hookState: 'notInstalled' })} tone='ghost'>
            All agents missing
          </Btn>
        </div>
        <div className='cp-agent-list'>
      ...
      <Row hint='another `gx` shadows ours' label='gx blocked'>
        <Toggle
          ...
          onChange={(next) => patchEnv({ ghostexCli: { ...env.ghostexCli, gxBlockedByExistingCommand: next } })}
        />
      </Row>
      ...
      <div className='cp-group-label'>
        ...
        <span className='cp-dim'>
          {' '}
          — {installedSkillCount}/{BUNDLED_SKILL_IDS.length} installed
        </span>
      </div>
      <div className='cp-btn-row'>
      ...
      <div className='cp-checkbox-grid'>
        ...
        <Toggle
          ...
          label='accessibility'
          onChange={(next) => patchEnv({ cuaDriver: { ...env.cuaDriver, accessibilityPermission: next } })}
        />
        <Toggle
          checked={env.cuaDriver.screenRecordingPermission}
          label='screen recording'
          onChange={(next) => patchEnv({ cuaDriver: { ...env.cuaDriver, screenRecordingPermission: next } })}
        />
      </div>
