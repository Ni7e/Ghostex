...
fn dispatch(argv: &[String]) -> CliResult<i32> {
    ...
    if !is_known_command(command_name) {
        if launchers::is_existing_bare_path_argument(command_name) {
            actions::run_bridge_action("openPaths", actions::Parser::OpenPaths, actions::BridgeOptions { fail_on_not_ok: true, assert_ok: false }, argv)?;
            return Ok(exit_code());
        }
...
fn is_known_command(name: &str) -> bool {
    const NAMES: [&str; 101] = [
        ...
        "restart-session", "fork-session", "reload-session", "rename-session",
        "sleep-session", "tag-session", "pin-session", "send-text", "send-enter",
        "send-key", "send-message", "message", "msg", "read-text", "read-messages",
        "read-thread", "wait-for-text", "rename-command", "set-visible-count",
        "set-view-mode", "open-browser", "open-browser-pane", "browser",
        "browser-devtools-mcp", "browser-mcp", "bd", "beads", "server",
        "install-browser-skill", "install-browser-mcp-skill", "computer-use",
        "install-computer-use-skill", "agent-orchestration",
        "install-agent-orchestration-skill", "fable-5.5-orchestration",
        "install-fable-5.5-orchestration-skill", "generate-title",
        "install-generate-title-skill", "manage-beads", "install-manage-beads-skill",
        "move-codex-session", "install-move-codex-session-skill", "toggle-sidebar",
        "move-sidebar", "assert-card", "wait-for", "screenshot", "logs", "bundle", "help",
    ];
...
fn run_command(name: &str, args: &[String]) -> CliResult<()> {
    ...
    let plain = BridgeOptions { fail_on_not_ok: false, assert_ok: false };
    let fail_on_not_ok = BridgeOptions { fail_on_not_ok: true, assert_ok: false };
    let assert_ok = BridgeOptions { fail_on_not_ok: false, assert_ok: true };
    match name {
        ...
        "kill" | "k" => wait::session_action_command("closeSession", "killed", &serde_json::json!({}), args),
        "sleep" => wait::session_action_command("sleepSession", "slept", &serde_json::json!({ "sleeping": true }), args),
        "wake" => wait::session_action_command("sleepSession", "woke", &serde_json::json!({ "sleeping": false }), args),
        ...
        "terminal" | "t" => run_bridge_action("createQuickTerminal", Parser::QuickTerminal, fail_on_not_ok, args),
        "create-session" => run_bridge_action("createSession", Parser::CreateSession, fail_on_not_ok, args),
        ...
        "acknowledge-session-attention" | "ack-session-attention" => {
            run_bridge_action("acknowledgeSessionAttention", Parser::SessionSelector, plain, args)
        }
        ...
        "move-project" => run_bridge_action("moveProject", Parser::ProjectMove, fail_on_not_ok, args),
        "add-project" => run_bridge_action("addProject", Parser::ProjectPath, plain, args),
        "remove-project" => run_bridge_action("removeProject", Parser::Project, fail_on_not_ok, args),
        "close-session" => run_bridge_action("closeSession", Parser::SessionSelector, plain, args),
        "restart-session" => run_bridge_action("restartSession", Parser::SessionSelector, plain, args),
        "fork-session" => wait::fork_session_command(args),
        "reload-session" => run_bridge_action("fullReloadSession", Parser::SessionSelector, plain, args),
        "rename-session" => run_bridge_action("renameSession", Parser::Rename, fail_on_not_ok, args),
        "sleep-session" => run_bridge_action("sleepSession", Parser::SessionBoolean("sleeping"), plain, args),
        "tag-session" => run_bridge_action("tagSession", Parser::SessionTag, plain, args),
        "pin-session" => run_bridge_action("pinSession", Parser::SessionBoolean("pinned"), plain, args),
        "send-text" => run_resolved_session_bridge_action("sendText", Parser::SendText, plain, args),
        "send-enter" => run_resolved_session_bridge_action("sendEnter", Parser::SessionSelector, plain, args),
        ...
        "rename-command" => run_resolved_session_bridge_action("renameCommand", Parser::Rename, plain, args),
        "set-visible-count" => run_bridge_action("setVisibleCount", Parser::VisibleCount, plain, args),
        ...
        "install-browser-skill" | "install-browser-mcp-skill" => skills::install_browser_skill_command(args),
        ...
        "install-agent-orchestration-skill" => skills::install_agent_orchestration_skill_command(args),
        "fable-5.5-orchestration" => skills::fable55_orchestration_command(args),
        "install-fable-5.5-orchestration-skill" => skills::install_fable55_orchestration_skill_command(args),
        ...
        "install-move-codex-session-skill" => skills::install_move_codex_session_skill_command(args),
        ...
        "assert-card" => run_bridge_action("assertSidebarCard", Parser::AssertCard, assert_ok, args),
