...
async function main(runOptions) {
  ...
  if (!(await isTcpPortAvailable(runOptions.port))) {
    ...
    throw new Error(`${LOCAL_HOST}:${runOptions.port} is already in use; stop the current gxserver on the selected port before running the compatibility harness.`);
  }
  ...
  try {
    ...
    recordObservation(observations, "minimalHealth", normalizeExchange({
    ...
    recordObservation(observations, "unauthorizedRpc", normalizeExchange({
    ...
    recordObservation(observations, "methodGate", normalizeExchange({
    ...
    recordObservation(observations, "missingProtocol", normalizeExchange({
    ...
    recordObservation(observations, "wrongProtocol", normalizeExchange({
    ...
    recordObservation(observations, "bodyProtocolRpc", normalizeExchange({
      request: {
        body: { params: {}, protocolVersion: PROTOCOL_VERSION },
        method: "POST",
    ...
    recordObservation(observations, "jsonBodyLimit", normalizeExchange({
      request: {
    ...
    recordObservation(observations, "authenticatedHealth", normalizeExchange({
    ...
    recordObservation(observations, "cliStatusRunning", normalizeValue({
    ...
    recordObservation(observations, "controlStop", normalizeExchange({
      request: {
        body: { protocolVersion: PROTOCOL_VERSION },
        method: "POST",
    ...
    recordObservation(observations, "cliStatusStopped", normalizeValue({
...
async function runPhase3DomainChecks({ homeDir, observations, token }) {
  ...
  recordObservation(observations, "phase3CreateProject", normalizeExchange({
    request: {
      body: {
        params: {
          identityIcon: { color: "blue", kind: "emoji", value: "G" },
  ...
  recordObservation(observations, "phase3UpdateProject", normalizeExchange({
    request: {
      body: {
        params: {
          customAgentOrder: ["codex"],
  ...
  recordObservation(observations, "phase3AddProjectPath", normalizeExchange({
    request: {
  ...
  recordObservation(observations, "phase3CreateSession", normalizeExchange({
    request: {
      body: {
        params: {
          cwd: projectDir,
  ...
  recordObservation(observations, "phase3CreateAgentSession", normalizeExchange({
    request: {
      body: {
        params: {
          agentId: "codex",
  ...
  recordObservation(observations, "phase3UpdateSession", normalizeExchange({
    request: {
      body: {
        params: {
          isPinned: true,
  ...
  recordObservation(observations, "phase3UpdateSessionOrder", normalizeExchange({
    request: {
      body: {
        params: {
          projectId: project.projectId,
  ...
  recordObservation(observations, "phase3ListProjects", normalizeExchange({
    request: {
      body: { params: {}, protocolVersion: PROTOCOL_VERSION },
      method: "POST",
  ...
  recordObservation(observations, "phase3ListSessions", normalizeExchange({
    request: {
      body: { params: { projectId: project.projectId }, protocolVersion: PROTOCOL_VERSION },
      method: "POST",
      path: "/api/listSessions",
  ...
  recordObservation(observations, "phase3ReadProjectStatus", normalizeExchange({
    request: {
      body: { params: { projectId: project.projectId }, protocolVersion: PROTOCOL_VERSION },
      method: "POST",
  ...
  recordObservation(observations, "phase3ReadPresentationSnapshot", normalizeExchange({
    request: {
      body: { params: {}, protocolVersion: PROTOCOL_VERSION },
      method: "POST",
  ...
  recordObservation(observations, "phase3SearchSessions", normalizeExchange({
    request: {
      body: {
        params: { limit: 10, projectId: project.projectId, query: "Terminal One Updated" },
        protocolVersion: PROTOCOL_VERSION,
  ...
  recordObservation(observations, "phase3RemoveSession", normalizeExchange({
    request: {
      body: {
        params: { projectId: project.projectId, sessionId: terminal.sessionId },
        protocolVersion: PROTOCOL_VERSION,
  ...
  recordObservation(observations, "phase3RemoveProject", normalizeExchange({
    request: {
...
async function runPhase4EventChecks({ homeDir, observations, token }) {
  ...
  try {
    ...
    recordObservation(observations, "phase4RendererUnavailable", normalizeExchange({
      request: {
        body: {
          params: {
            action: "toggleSidebarCollapsed",
    ...
    recordObservation(observations, "phase4RendererCommand", normalizeValue({
      event: commandEvent,
  ...
  recordObservation(observations, "phase4RevisionWithoutClients", normalizeValue({
...
async function runPhase5ZmxChecks({ homeDir, observations, token }) {
  ...
  const forgetLive = (projectId, sessionId) => {
    const index = liveSessions.findIndex((session) => session.projectId === projectId && session.sessionId === sessionId);
    if (index >= 0) {
  ...
  try {
    ...
    recordObservation(observations, "phase5StartPlainProvider", normalizeValue({
    ...
    recordObservation(observations, "phase5ProbeExists", normalizeValue({
    ...
    recordObservation(observations, "phase5ReadSessionText", normalizeValue({
    ...
    recordObservation(observations, "phase5SendSessionText", normalizeValue({
    ...
    recordObservation(observations, "phase5SendSessionEnter", normalizeValue({
    ...
    recordObservation(observations, "phase5SendSessionMessage", normalizeValue({
    ...
    recordObservation(observations, "phase5FocusWithoutRenderer", normalizeExchange({
      request: {
        body: {
          params: { projectId: project.projectId, sessionId: session.sessionId },
          protocolVersion: PROTOCOL_VERSION,
    ...
    recordObservation(observations, "phase5TargetlessMessageWithoutRenderer", normalizeExchange({
      request: {
        body: {
    ...
    recordObservation(observations, "phase5SleepSession", normalizeValue({
    ...
    recordObservation(observations, "phase5WakeSession", normalizeValue({
    ...
    rememberLive(project.projectId, session.sessionId);
    recordObservation(observations, "phase5RestartWithStartupText", normalizeValue({
    ...
    forgetLive(project.projectId, session.sessionId);
    recordObservation(observations, "phase5TransitionClose", normalizeValue({
    ...
    recordObservation(observations, "phase5OversizedSendRejected", normalizeExchange({
      request: {
        body: {
          params: {
            projectId: project.projectId,
...
async function runPhase6AgentChecks({ homeDir, observations, paths, token }) {
  ...
  const createProject = await requestJson("/api/createProject", {
    body: {
      params: {
        customAgents: [
          { agentId: "codex", command: "codex", name: "Codex" },
        ],
        name: "Phase 6 Agents",
        path: projectDir,
      },
  ...
  recordObservation(observations, "phase6LaunchPlanSettingsOff", normalizeValue(launchPlanOff.body.result.plan, homeDir));
  ...
  recordObservation(observations, "phase6RenameTerminal", normalizeValue({
  ...
  recordObservation(observations, "phase6ForkTerminalRejected", normalizeExchange({
    request: {
      body: {
        params: { projectId: project.projectId, sessionId: terminal.sessionId },
        protocolVersion: PROTOCOL_VERSION,
  ...
  recordObservation(observations, "phase6ResumePlan", normalizeValue({
  ...
  recordObservation(observations, "phase6RenameAgentPending", normalizeValue({
  ...
  recordObservation(observations, "phase6IngestSessionState", normalizeValue({
  ...
  recordObservation(observations, "phase6TerminalTitleCapture", normalizeValue({
  ...
  recordObservation(observations, "phase6UpdateAgentActivity", normalizeValue({
  ...
  recordObservation(observations, "phase6IngestAgentHookEvent", normalizeValue({
  ...
  recordObservation(observations, "phase6CancelFirstPromptAutoTitle", normalizeValue({
  ...
  recordObservation(observations, "phase6ReadAgentHookStatus", normalizeValue({
  ...
  recordObservation(observations, "phase6InstallAgentHooks", normalizeValue({
  ...
  recordObservation(observations, "phase6LogPrivacy", { rawPromptLogged: false, rawTitleLogged: false, rawAgentSessionIdLogged: false });
}
...
async function runPhase7TypedOperationChecks({ homeDir, observations, paths, runOptions, token }) {
  ...
  recordObservation(observations, "phase7RunGitStatus", normalizeValue({
  ...
  const commitPlan = await requestJson("/api/runGitAction", {
    body: {
      params: { action: "commit", messageBody: "private body", messageSubject: "private subject", noVerify: true, projectId: project.projectId },
      protocolVersion: PROTOCOL_VERSION,
    },
    method: "POST",
    token,
  ...
  recordObservation(observations, "phase7RunGitCommitRedaction", normalizeValue({
  ...
  recordObservation(observations, "phase7RunGitHubVersion", normalizeValue({
  ...
  const worktreeExists = await requestJson("/api/runWorktreeAction", {
    body: {
      params: { action: "pathExists", projectId: project.projectId, worktreePath: path.join(workspaceDir, "phase7-project-copy") },
      protocolVersion: PROTOCOL_VERSION,
    },
    method: "POST",
    token,
  ...
  recordObservation(observations, "phase7RunWorktreeList", normalizeValue({
    action: worktreeList.body.result.action,
    exitCode: worktreeList.body.result.exitCode,
    worktreeCount: Array.isArray(worktreeList.body.result.worktrees) ? worktreeList.body.result.worktrees.length : 0,
  }, homeDir));
  ...
  recordObservation(observations, "phase7TypedScopeRejected", normalizeExchange({
    request: {
      body: { params: { action: "status", projectPath: path.join(workspaceDir, "unregistered") }, protocolVersion: PROTOCOL_VERSION },
      method: "POST",
  ...
  recordObservation(observations, "phase7PreviewRepositoryClone", normalizeValue({
  ...
  recordObservation(observations, "phase7StartRepositoryClone", normalizeValue({
  ...
  recordObservation(observations, "phase7CloneExistingRejected", normalizeExchange({
    request: {
      body: { params: cloneParams, protocolVersion: PROTOCOL_VERSION },
      method: "POST",
...
function resolveTarget(runOptions) {
  ...
  if (runOptions.target === "rust") {
    const binaryPath = runOptions.bin ?? process.env.GHOSTEX_GXSERVER_RUST_BIN ?? path.join(gxserverRsRoot, "target", "debug", "gxserver");
...
async function waitForFileText(filePath, timeoutMs, child, output) {
  ...
  }, timeoutMs, `Timed out waiting for ${filePath}.`);
}
...
async function waitForServerReady(token, timeoutMs, child, output) {
  ...
  }, timeoutMs, "Timed out waiting for authenticated gxserver health.");
}
...
function assertChildStillUseful(child, output) {
  if (child.exitCode !== null) {
    throw new Error(`gxserver foreground exited early with code ${child.exitCode}.\nstdout:\n${output.stdout}\nstderr:\n${output.stderr}`);
  }
}
...
async function waitForProcessExit(child, timeoutMs, output) {
  ...
  try {
    await Promise.race([
      ...
      new Promise((_, reject) => {
        timeout = setTimeout(() => {
          reject(new Error(`Timed out waiting for gxserver foreground exit.\nstdout:\n${output.stdout}\nstderr:\n${output.stderr}`));
        }, timeoutMs);
      }),
    ]);
  } finally {
...
async function updateOrCompareFixture(runOptions, observations) {
  ...
  if (runOptions.updateFixtures) {
    ...
    await writeFile(observedTsFixturePath, `${JSON.stringify({
