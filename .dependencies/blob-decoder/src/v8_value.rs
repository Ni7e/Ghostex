...
impl<'a> Reader<'a> {
    ...
    // V8 inserts zero padding before UTF-16 strings to align their payload.
    // Consume it only at tag boundaries, never inside scalar payloads.
    ...
    fn read_tag(&mut self) -> Result<u8, V8Error> {
        self.skip_padding();
        self.read_u8()
    }
