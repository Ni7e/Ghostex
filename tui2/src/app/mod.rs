...
pub struct App {
    ...
    pub(crate) next_ghostex_session_refresh: Option<Instant>,
...
impl App {
    pub fn new(
        ...
        let mut state = AppState {
            ...
            request_ghostex_attach_session_key: None,
            request_ghostex_create_terminal: false,
            request_ghostex_create_terminal_target: None,
            ...
            ghostex_mode: false,
            ghostex_sessions: Vec::new(),
            ghostex_active_session_key: None,
            ghostex_session_workspaces: std::collections::HashMap::new(),
            ghostex_status: None,
            tab_scroll: 0,
        ...
        Self {
            ...
            next_agent_manifest_update_check: auto_updates_enabled(no_session)
                .then_some(Instant::now() + AUTO_UPDATE_CHECK_INTERVAL),
            next_ghostex_session_refresh: None,
    ...
    pub async fn run(&mut self, terminal: &mut DefaultTerminal) -> io::Result<()> {
        ...
        while !self.state.should_quit {
            ...
            if self.maybe_refresh_ghostex_sessions(now) {
                needs_render = true;
            }
            ...
            if let Some(key) = self.state.request_ghostex_attach_session_key.take() {
                self.attach_ghostex_session_by_key(&key);
                needs_render = true;
            }

            if let Some((project_id, group_id)) =
                self.state.request_ghostex_create_terminal_target.take()
            {
                self.create_ghostex_terminal_for(project_id, group_id);
                needs_render = true;
            }

            if self.state.request_ghostex_create_terminal {
                self.state.request_ghostex_create_terminal = false;
                self.create_ghostex_terminal();
                needs_render = true;
            }
    ...
    pub(crate) fn ensure_default_workspace(&mut self) -> bool {
        if self.state.ghostex_mode && self.state.workspaces.is_empty() {
            return false;
        }
