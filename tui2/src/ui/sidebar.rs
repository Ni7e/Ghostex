...
const GHOSTEX_SIDEBAR_HEADER_ROWS: u16 = 1;
const GHOSTEX_SIDEBAR_FOOTER_ROWS: u16 = 1;

pub(crate) struct AgentPanelEntry {
    pub ws_idx: usize,
    pub tab_idx: usize,
    pub pane_id: crate::layout::PaneId,
    pub ghostex_session_key: Option<String>,
...
fn agent_panel_entries_with_runtimes(
    ...
    if app.ghostex_mode {
        return app
            ...
            .iter()
            .map(|session| {
                let (state, seen) = crate::ghostex::session_agent_state(session);
                AgentPanelEntry {
                    ws_idx: 0,
                    tab_idx: 0,
                    pane_id: crate::layout::PaneId::from_raw(0),
                    ghostex_session_key: Some(crate::ghostex::session_identity_key(session)),
                    primary_label: crate::ghostex::project_label(session),
                    primary_tab_label: Some(crate::ghostex::session_title(session)),
                    agent_label: crate::ghostex::normalized_agent_label(session),
                    state,
                    seen,
                    custom_status: session
                        ...
                        .unwrap_or(false)
                        .then(|| "favorite".into()),
                    state_labels: crate::ghostex::session_state_labels(session),
                }
            })
            .collect();
    }
...
pub(crate) fn agent_panel_scroll_metrics(app: &AppState, area: Rect) -> crate::pane::ScrollMetrics {
    if app.ghostex_mode {
        return ghostex_sidebar_scroll_metrics(app, area);
    }
...
pub(crate) fn agent_panel_scrollbar_rect(app: &AppState, area: Rect) -> Option<Rect> {
    if app.ghostex_mode {
        return ghostex_sidebar_scrollbar_rect(app, area);
    }
...
pub(crate) fn ghostex_sidebar_rect(area: Rect) -> Rect {
    let content = Rect::new(area.x, area.y, area.width.saturating_sub(1), area.height);
    if content.width == 0 || content.height == 0 {
        Rect::default()
    } else {
...
pub(crate) fn ghostex_sidebar_body_rect(area: Rect, has_scrollbar: bool) -> Rect {
    if area.width == 0 || area.height <= GHOSTEX_SIDEBAR_HEADER_ROWS + GHOSTEX_SIDEBAR_FOOTER_ROWS {
        return Rect::default();
    }

    let body_y = area.y.saturating_add(GHOSTEX_SIDEBAR_HEADER_ROWS);
    let body_bottom = area
        .y
        .saturating_add(area.height)
        .saturating_sub(GHOSTEX_SIDEBAR_FOOTER_ROWS);
    Rect::new(
        area.x,
        body_y,
...
fn ghostex_sidebar_visible_count(area: Rect) -> usize {
    ghostex_sidebar_body_rect(area, false).height as usize
}

pub(crate) fn ghostex_sidebar_scroll_metrics(
    app: &AppState,
    area: Rect,
) -> crate::pane::ScrollMetrics {
    let total_rows = crate::ghostex::sidebar_rows(&app.ghostex_sessions).len();
    let viewport_rows = ghostex_sidebar_visible_count(area).min(total_rows);
    let max_offset_from_bottom = total_rows.saturating_sub(viewport_rows);
...
pub(crate) fn ghostex_sidebar_scrollbar_rect(app: &AppState, area: Rect) -> Option<Rect> {
    let metrics = ghostex_sidebar_scroll_metrics(app, area);
    let body = ghostex_sidebar_body_rect(area, true);
    (should_show_scrollbar(metrics) && body.width > 0 && body.height > 0).then_some(Rect::new(
        area.x + area.width.saturating_sub(1),
        body.y,
        1,
...
pub(crate) fn ghostex_sidebar_row_at_screen_row(
    app: &AppState,
    area: Rect,
    row: u16,
) -> Option<crate::ghostex::SidebarRow> {
    let metrics = ghostex_sidebar_scroll_metrics(app, area);
    let body = ghostex_sidebar_body_rect(area, should_show_scrollbar(metrics));
    ...
    crate::ghostex::sidebar_rows(&app.ghostex_sessions)
        .get(row_idx)
        .cloned()
}
...
pub(super) fn render_sidebar(
    ...
    if app.ghostex_mode {
        render_ghostex_sidebar(app, frame, ghostex_sidebar_rect(area));
...
fn render_ghostex_sidebar(app: &AppState, frame: &mut Frame, area: Rect) {
    ...
    let rows = crate::ghostex::sidebar_rows(&app.ghostex_sessions);
    let metrics = ghostex_sidebar_scroll_metrics(app, area);
    let scrollbar_rect = ghostex_sidebar_scrollbar_rect(app, area);
    let body = ghostex_sidebar_body_rect(area, should_show_scrollbar(metrics));
    if body != Rect::default() {
            ...
            .enumerate()
        {
            ...
            let line = ghostex_sidebar_row_line(app, row, body.width);
            let row_bg = ghostex_sidebar_row_bg(app, row);
            ...
            frame.render_widget(
                Paragraph::new(line)
                    .style(row_bg.map_or_else(Style::default, |bg| Style::default().bg(bg))),
    ...
    if let Some(status) = app.ghostex_status.as_deref() {
        ...
        frame.render_widget(
            Paragraph::new(format!(
                " {}",
                truncate_text(status, area.width.saturating_sub(1) as usize)
            ))
...
fn ghostex_sidebar_row_bg(app: &AppState, row: &crate::ghostex::SidebarRow) -> Option<Color> {
    match row {
        crate::ghostex::SidebarRow::Session(session)
            if app.ghostex_active_session_key.as_deref()
                == Some(crate::ghostex::session_identity_key(session).as_str()) =>
        {
...
fn ghostex_sidebar_row_line<'a>(
    app: &AppState,
    row: &'a crate::ghostex::SidebarRow,
    width: u16,
) -> Line<'a> {
    let p = &app.palette;
    let bg = ghostex_sidebar_row_bg(app, row).unwrap_or(Color::Reset);
    match row {
        crate::ghostex::SidebarRow::Project(project) => {
            let label = truncate_text(&project.name, width.saturating_sub(1) as usize);
        ...
        crate::ghostex::SidebarRow::NewTerminal { .. } => Line::from(vec![Span::styled(
            truncate_text("    Create new terminal", width as usize),
            Style::default().fg(p.subtext0).bg(bg),
        )]),
        crate::ghostex::SidebarRow::Session(session) => {
            let active = app.ghostex_active_session_key.as_deref()
                == Some(crate::ghostex::session_identity_key(session).as_str());
            let title_width = width.saturating_sub(10) as usize;
            let title = truncate_text(&crate::ghostex::session_title(session), title_width);
            ...
            Line::from(vec![
                ghostex_activity_dot_span(app, session, bg),
                Span::styled(
                    format!("  [{}] ", crate::ghostex::agent_indicator(session)),
                    Style::default()
                        .fg(ghostex_agent_color(app, session))
                        .bg(bg),
                ),
...
fn ghostex_activity_dot_span<'a>(
    app: &AppState,
    session: &crate::ghostex::SessionItem,
    bg: Color,
) -> Span<'a> {
    match crate::ghostex::session_activity(session) {
        Some(crate::ghostex::SessionActivity::Attention) => {
            Span::styled(" ●", Style::default().fg(app.palette.green).bg(bg))
        }
        Some(crate::ghostex::SessionActivity::Working) => {
            Span::styled(" ●", Style::default().fg(app.palette.yellow).bg(bg))
        }
        None => Span::styled("  ", Style::default().bg(bg)),
    }
}

fn ghostex_agent_color(app: &AppState, session: &crate::ghostex::SessionItem) -> Color {
    match crate::ghostex::normalized_agent_key(session).as_str() {
        "agy" | "anti-gravity" | "anti-gravity-cli" | "antigravity" | "antigravity-cli"
        | "gemini" => app.palette.mauve,
        "claude" | "claude-code" | "claude-work" => app.palette.peach,
...
fn render_agent_detail(
    ...
    if !app.ghostex_mode && toggle_rect != Rect::default() {
        frame.render_widget(
    ...
    for detail in details.iter().skip(app.agent_panel_scroll) {
        ...
        let is_active = detail
            .ghostex_session_key
            .as_ref()
            .is_some_and(|key| app.ghostex_active_session_key.as_deref() == Some(key.as_str()))
            || (detail.ghostex_session_key.is_none()
