...
pub const Surface = struct {
    ...
    write_pty: ?*const fn (?*anyopaque, [*]const u8, usize) callconv(.c) void = null,
    ...
    pub const Options = extern struct {
        ...
        /// Callback for PTY input bytes generated by Ghostty input handling.
        ///
        /// CDXC:iOSNativeTerminals 2026-05-22-10:49:
        /// iOS owns command and SSH streams outside Ghostty, but native text,
        /// paste, and key handling must still pass through Ghostty before bytes
        /// reach those streams. Embedded surfaces expose the final PTY writes
        /// back to the host so the app does not bypass Ghostty or revive hterm.
        write_pty: ?*const fn (?*anyopaque, [*]const u8, usize) callconv(.c) void = null,
    ...
    pub fn writePtyData(self: *Surface, data: []const u8, linefeed: bool) bool {
        const func = self.write_pty orelse return false;
        ...
        var buf_i: usize = 0;
        for (data) |ch| {
            const needed: usize = if (ch == '\r') 2 else 1;
            if (buf_i + needed > buf.len) {
                func(self.userdata, &buf, buf_i);
                buf_i = 0;
            }
            if (ch == '\r') {
                buf[buf_i] = '\r';
        ...
        if (buf_i > 0) func(self.userdata, &buf, buf_i);
        return true;
    }
...
pub const CAPI = struct {
    ...
    /// Feed remote process output into the surface terminal and renderer.
    ///
    /// CDXC:iOSNativeTerminals 2026-05-22-10:42:
    /// The iOS app owns command and SSH streams outside Ghostty, so embedded
    /// GhosttyKit surfaces must accept already-read terminal bytes directly.
    /// Use Termio.processOutput so iOS output follows the same parser,
    /// scrollback, damage, and Metal render scheduling path as Ghostty's PTY
    /// reader instead of adding a JavaScript terminal fallback.
    export fn ghostty_surface_write_data(
        surface: *Surface,
        ptr: [*]const u8,
        len: usize,
    ) void {
