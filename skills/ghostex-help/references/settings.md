...
- **Dark theme** `darkThemePreset` (one of gray | black | blue | green | red | purple | custom; default gray): Preset dark chrome for the sidebar and window, or Custom to tune its contrast and tint. Option labels: gray = Dark Gray, black = Black, blue = Blue, green = Green, red = Red, purple = Purple, custom = Custom.
...
- **Storage usage** `storageUsage` (Settings UI row without a settings key; use `ghostex settings open`): Inspect browser storage by feature, budgets, pending saves, and disposable caches.
- **Ghostex folder storage** `storageStats` (Settings UI row without a settings key; use `ghostex settings open`): View on-disk Ghostex folder sizes and total usage, refresh statistics, or open the data folder.
## About (tab `about`)
