...
- **Glass shows** `windowGlassSource` (one of desktopAndWindows | wallpaper | customImage; default desktopAndWindows): Wallpaper only keeps other windows from showing through the glass. Custom image shows a picture you choose. Option labels: desktopAndWindows = Desktop and windows, wallpaper = Wallpaper only, customImage = Custom image.
- **Glass picture position** `windowGlassImagePlacement` (one of static | desktop; default static): Where the wallpaper or custom picture sits behind the glass. Stays with the desktop can trail the window while you drag it. Option labels: static = Moves with the window, desktop = Stays with the desktop.
...
- **Chat brain** `chatBrain` (one of quickjs | rust; default quickjs): Which engine runs the chat. Leave this on QuickJS; the Rust engine is still in development. Option labels: quickjs = QuickJS (shipped), rust = Rust (in development).
