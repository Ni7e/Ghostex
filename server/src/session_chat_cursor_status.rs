...
use crate::agent_hooks::statusline::{read_statusline_payload, StatuslineAgent};
...
/// Cursor's stored payload for this Ghostex session, as a detection layer:
/// the context usage and the `cursorStatus` rows. `None` without a payload.
pub fn read_cursor_statusline_selection(
    ...
    let stored = read_statusline_payload(
        hook_state_directory,
        StatuslineAgent::Cursor,
        &agent_session_id,
    )?;
    let project = repository.get_project(project_id).ok().flatten();
    let git = effective_session_git_cwd(&session, project.as_ref())
        .as_deref()
        .and_then(session_git_status);
    ...
    let status = cursor_status_value(payload, git.as_ref());
...
fn cursor_status_value(
    ...
    put("projectDir", text(&["workspace", "project_dir"]));
    ...
    // Cursor only writes `max_mode` while it is on.
    put(
        "maxMode",
        Some(Value::Bool(
            payload
