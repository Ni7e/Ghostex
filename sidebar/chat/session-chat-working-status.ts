...
export function lifecycleTerminatesCurrentTurn(
  ...
  if (lifecycle.timestamp > 1e11 && workingStartedAt > 1e11) {
    ...
    return (
      lifecycle.timestamp + SESSION_CHAT_LIFECYCLE_CLOCK_SKEW_SLACK_MS >=
      workingStartedAt
    );
  }
  return false;
...
export function trailingAssistantPostDates(
  ...
  return (
    last?.role === "assistant" &&
    last.timestamp != null &&
    last.timestamp >= workingStartedAt
  );
}
...
export function deriveSessionChatWorkingOverride(
  input: SessionChatWorkingInput,
): "working" | undefined {
  if (!input.working) {
    return undefined;
  }
...
export function mergeSessionChatStatus(
  input: SessionChatStatusMergeInput,
): SessionChatStatus {
  if (input.error) {
    return "error";
  }
...
export function shouldShowSessionChatWorking(input: {
  working: boolean;
  interrupted: boolean;
}): boolean {
...
export function shouldClearSessionChatWorkingSuppression(input: {
  working: boolean;
}): boolean {
