...
function Pane({
  ...
  const dropTab = (event: DragEvent, insertionIndex: number) => {
    ...
    if (dragged) {
      onChange(
        moveWorkspaceTab(model, dragged.paneId, leaf.paneId, dragged.sessionId, insertionIndex),
      );
    }
  };
            ...
            return (
              <AppTooltip content={session.title} key={tab.sessionId}>
                ...
                <button
                  ...
                  onClick={(event) => {
                    event.stopPropagation();
                    onChange(closeWorkspaceTab(selectWorkspaceTab(model, leaf.paneId, tab.sessionId), leaf.paneId, tab.sessionId));
                  }}
...
export function AgentsWorkspace({
  ...
  const authoritativeMachines = useMemo(
    () => new Set(authoritativeMachineIds),
    [authoritativeMachineIds],
  );

  useEffect(() => {
    setModel((current) => integrated
    ...
    setModel((current) => {
      ...
      if (existingPane) {
        return selectWorkspaceTab(
          updateWorkspaceSession(current, openRequest.session),
          existingPane.paneId,
          sessionId,
        );
      }
  ...
  const createTerminal = (paneId: string, splitAxis?: WorkspaceSplitAxis) => {
    ...
    update(
      splitAxis
        ? splitWorkspacePane(model, paneId, splitAxis, pending)
        : addWorkspaceSession(model, paneId, pending),
    );
  ...
  const handleMenu = (
    paneId: string,
    action: "split-horizontal" | "split-vertical" | "rotate" | "merge" | "focus",
  ) => {
  ...
  const visibleRoot = model.focusModePane
    ? workspaceLeaf(model, model.focusModePane) ?? model.root
    : model.root;
  ...
  return (
    ...
    >
      ...
      {model.focusModePane && (
        <button
          className="workspace-exit-focus"
          onClick={() => update(toggleWorkspaceFocusMode(model))}
          type="button"
        >
          Exit Focus
        </button>
        ...
        >
          <button
            onClick={() => {
              update(
                toggleWorkspaceFocusMode(
                  selectWorkspaceTab(model, tabMenu.paneId, tabMenu.sessionId),
                  tabMenu.paneId,
                ),
              );
              setTabMenu(null);
