...
Slice 204 reconciles Phase 4 as source-side command runtime status, not acceptance. Command terminals use separate command host/native view/Ghostty state with command-scoped body bounds, focus, close, process-exit, input, and final-close-collapse paths; these maps must stay isolated from Agents and startup runtime state. Command launch payload production now has explicit terminal Action and plain command-terminal project-cwd sources with exact-slot one-shot consumption, while user/runtime validation remains separate; slice 489 adds current Action run-start/status-file lifecycle evidence and slice 253 accepts current physical-key/product behavior for the source ledger, while validation stays outside this agent workflow.

CDXC:GPUICommandTerminalRuntime 2026-06-27-05:07:
Slice 489 sharpens the Phase 4 handoff around command launch and status source boundaries. Command terminal surfaces may mount through the command-only host/native view/Ghostty pipeline with command-scoped body bounds, and the Ghostty launch request may receive only an explicit terminal Action payload or plain command-terminal project cwd payload consumed by the exact command body slot/runtime key. Requests without an exact explicit payload stay intentionally empty. Action-owned command tabs may move through Working/Idle via run ids plus the env-provided session-state status file; non-Action/restored idle, working, attention, and delayed-send status remains shell enum/boolean metadata for tab chrome and persistence only. Neither path may infer command text, cwd, env, stdout/stderr, terminal content, delayed-send deadlines, status, or launch data from titles, paths, logs, status-file paths, or shell JSON.

CDXC:GPUICommandTerminalLaunchPayloadSource 2026-06-23-16:46:
Slice 217 adds the command launch payload boundary separate from Action status tracking. The command Ghostty config path now consults explicit payloads keyed by exact command body mount slot plus derived command runtime id; terminal Action and plain command-terminal project-cwd producers may attach only through that exact key and are consumed once during Ghostty config preparation. Invalid explicit payloads suppress/prune the config request and never fall back to titles, status, delayed-send state, project/workspace data, paths, cwd, command args, env, initial input, wait policy, stdout/stderr, terminal content, or helper detection.
...
CDXC:GPUIWorkspaceParityDocsAlignment 2026-06-24-08:40:
Slice 294 is docs-only Phase 4 command-terminal wording cleanup. Command terminal source evidence is accepted for this source-ledger pass as real terminal evidence, explicit command launch producers now exist for terminal Actions and plain command-terminal project cwd, and slice 489 supersedes the old Action status gap with run ids, status-file stamping/polling, completion feedback, and exit cleanup; slice 312 supersedes earlier runtime-proof wording so command terminal runtime proof/checks are not queued for this pass.

CDXC:GPUICommandTerminalRuntime 2026-06-24-09:29:
Slice 312 updates Phase 4 for the latest source-only runtime-parity rule. Terminal and command terminal are accepted as working for this pass, so Phase 4 must preserve current explicit command launch producer limits, Action status-file lifecycle limits, remaining user/runtime validation caveats, future non-Action/generalized status product decisions, clipboard/physical-key limits, shell-only transfer caveats, and no-validation evidence without listing command terminal runtime proof/checks as active remaining work; Source/Browser/Kanban/Manage runtime checks remain user-side.

CDXC:GPUICommandTerminalLaunchPayloadSource 2026-06-27-04:59:
Slice 488 refreshes the Phase 4 handoff after exact-slot one-shot command launch producers landed. Terminal Actions and plain command-terminal project cwd are current explicit launch payload sources, not future-only work; slice 489 separately records current Action status-file lifecycle evidence, and remaining caveats are user/runtime validation, non-Action/generalized command status product decisions, and privacy/no-inference/no-fallback constraints.

CDXC:GPUICommandPaneActions 2026-06-27-05:07:
Slice 489 records the current Action command status source: GPUI owns live run ids, env-provided session-state files, status-file stamping/polling, run-start Working state, idle completion, completion feedback, and exit cleanup. Non-Action/restored command status remains safe enum/boolean metadata, and status must not be inferred from titles, command text, cwd/env, stdout/stderr, terminal content, paths, logs, status-file paths, or shell JSON.
-->
...
Current accepted command source-ledger state after slice 489:
...
- The command-mounted surface path uses the shared host/native view/Ghostty maps and an exact-slot one-shot launch payload boundary keyed by exact command body mount slot plus derived command runtime id. Terminal Actions and plain command-terminal creation can produce explicit payloads; each payload is consumed once at Ghostty config preparation for that exact slot/runtime key, and requests without an exact matching payload remain payload-less.
- Explicit payloads may attach only through that exact key after Ghostty launch-payload validation. Invalid explicit payloads are drained and suppress/prune the config request instead of falling back to command titles, shell status, delayed-send booleans, project/workspace data, paths, cwd, command args, env, initial input, wait policy, delayed-send deadlines, stdout/stderr, terminal content, or helper detection.
...
- Action-owned command tabs have current run-start/status-file lifecycle evidence: live run ids, env-provided session-state files, status-file stamping/polling, Working state, idle completion, completion feedback, and exit cleanup. Non-Action/restored idle/working/attention/delayed-send status remains privacy-safe shell enum/boolean metadata for tab chrome and persistence only, and neither path may infer command text, cwd, env, stdout/stderr, terminal content, delayed-send deadlines, status, or launch data from titles, paths, logs, status-file paths, or shell JSON.
...
- Command terminal runtime ids, host/surface handles, command input, paths, stdout/stderr, delayed-send deadlines, launch data when present, and terminal content remain runtime-only or absent and must not enter shell-state JSON or persistent logs.
...
- Accepted command-body terminal surface mount evidence.
  - Source status: represented through command-only host/native view/Ghostty maps with command-scoped body bounds, while preserving separate command layout/state, keeping command surfaces out of normal Agents runtime maps, and consulting the exact-slot one-shot launch-payload source for explicit terminal Action or plain command-terminal project cwd payloads.
- Accepted command config boundary evidence with explicit launch producers and current Action status-file lifecycle limits.
  - Source status: surface mounting is represented by the command host/native view/Ghostty pipeline, and command config preparation has an exact-slot one-shot explicit payload boundary. Terminal Action payloads and plain command-terminal project-cwd payloads are represented as current producers; user/runtime validation and any additional launch payload families remain product decisions rather than current source-ledger blockers. Requests without exact explicit payloads do not launch from command text, cwd, env, stdout/stderr, terminal content, status labels, project/workspace data, helper detection, or delayed-send metadata.
- Accepted command Action status-file and semantic metadata evidence.
  - Source status: Action-owned command tabs have live run ids, session-state status-file stamps, a bounded status poller, run-start Working, idle completion, completion feedback, and exit cleanup. Non-Action/restored status remains only idle/working/attention enum plus delayed-send boolean metadata for tab chrome and shell-state persistence. Neither path is an input to launch, cwd/env, output, terminal content, delayed-send deadline, or process inference.
...
- Remaining command launch caveats are user/runtime validation and any new launch payload families beyond existing terminal Action and plain command-terminal project-cwd producers. GPUI must keep exact-slot one-shot consumption and must not infer launch data from titles, shell status, delayed-send state, paths, logs, stdout/stderr, terminal content, project names, or helper detection.
- Future command status product work is limited to non-Action/generalized status beyond the current Action run-id/status-file lifecycle and safe restored metadata. It must keep status-file paths, command text, cwd/env, stdout/stderr, terminal content, logs, titles, and shell JSON out of persistence and inference.
