...
<!-- CDXC:Distribution 2026-06-18-06:14: The 4.20.0 public feature index must summarize the simplified setup flow, tutorial video help, broader command palette, hook uninstall recovery controls, title-only session search, native pane polish, delayed-send focus, CLI prompt preservation, and refreshed README/gallery docs. -->
<!-- CDXC:Distribution 2026-06-19-06:40: The 4.20.1 public feature index must summarize the titlebar Settings menu move, real Chromium color-scheme emulation, debounced Source settings restarts, and pane action icon polish. -->
<!-- CDXC:Distribution 2026-06-19-10:23: The 4.21.0 public feature index must summarize user-facing work since v4.20.0 and include external human contributor credit where release notes mention those fixes. -->
...
- 4.21.1 makes `ghostex create-agent` start its gxserver provider immediately, consumes queued fresh-agent startup text without treating Ghostex session IDs as resume titles, adds command-palette and Hotkeys rows for focused-session Sleep, Wake, Close, and Close After Done with Option+Shift+S for Sleep, keeps those actions scoped to the terminal that owns native focus, quiets the Git commit modal branch/files footer, and fixes Chromium browser panes so transparent public pages use a white light-mode canvas instead of the dark editor backing.
- 4.21.0 installs bundled Ghostex agent skills through gxserver's shared skill installer, detects skills across global and repository skill roots, moves Default Prompt Agent ownership into gxserver, persists Browser System/Light/Dark choices per project and web origin, simplifies titlebar Settings and Hotkeys shortcuts, removes duplicate native tab overflow actions, adds calibrated dark sidebar/titlebar tint presets with a continuous native gradient, makes reference-sidebar Search inline, improves manual session drag/drop indicators and gap right-click session menus, waits for Source-pane code-server readiness before first navigation, restores Delayed Send timers from saved remaining-duration checkpoints, adds cleaner scroll fades across sidebar and menu surfaces, keeps prompt-editor timing diagnostics privacy-safe, speeds up Project Board label suggestions by deriving them from loaded issues, brightens Kanban cards, tightens swimlane spacing, skips first-launch hook warnings when a primary provider is ready, and includes Browser initial-URL plus section-header overlap fixes from @cuttothechaseo.
