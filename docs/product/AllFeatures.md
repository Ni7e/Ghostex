...
- 5.4.0 fixes sidebar lag by cutting repeated gxserver Git checks, duplicate native status updates, and fast zmx title-observer retries; adds sidebar multi-select bulk session actions; expands GPUI parity with startup feedback, toasts, configured hotkeys, folder picking, Close After Done, previous-session text search, and Ghostty runtime actions; makes Manage HTML previews fully interactive; and restores embedded T3 Code sessions to their bound threads more reliably.
