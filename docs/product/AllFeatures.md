...
- 4.21.0 installs bundled Ghostex agent skills through gxserver's shared skill installer, detects skills across global and repository skill roots, moves Default Prompt Agent ownership into gxserver, persists Browser System/Light/Dark choices per project and web origin, simplifies titlebar Settings and Hotkeys shortcuts, removes duplicate native tab overflow actions, speeds up Project Board label suggestions by deriving them from loaded issues, brightens Kanban cards, tightens swimlane spacing, skips first-launch hook warnings when a primary provider is ready, and includes Browser initial-URL plus section-header overlap fixes from @cuttothechaseo.
