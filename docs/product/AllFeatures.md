...
- 6.5.0 publishes the new shared React Native mobile app as a signed Android APK and begins its iOS rollout through TestFlight; adds mobile long-press session actions, a cleaner terminal header and tab bar, clearer status indicators, steadier IME and terminal sizing, richer remote-session details, CLI controls for Delayed Send and Close After Done, easier-to-read project panels, and more reliable persistent-session refreshes.
