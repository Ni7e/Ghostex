...
describe('phased macOS code-server prerequisite contract', () => {
  ...
  test('downloads both archives from the exact release-state prerequisite runs', () => {
    ...
    expect(source).toContain('.release-automation/scripts/release-gpui/verify-code-server-archive.mjs');
  ...
  test('keeps every other active macOS release entry path fail-closed', () => {
    ...
    expect(reusableWorkflow).toContain('scripts/release-gpui/verify-code-server-archive.mjs');
  });
...
describe('active WSL2 code-server consumer contract', () => {
  ...
  test.each([
    ...
    const archiveResult = spawnSync(path.resolve('scripts/release-gpui/create-deterministic-tar.sh'), [
      payload,
      archive,
    ]);
