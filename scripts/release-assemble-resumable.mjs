...
const { completed, release: initialRelease, state } = validateStagedRelease(version, {
...
try {
  const deliverables = state.expected.map((name) => {
    ...
    const response = run("gh", ["release", "download", `v${version}`, "--repo", RELEASE_REPO, "--pattern", name, "--dir", temporary], { allowFailure: true });
  ...
  try {
    ...
    const bundleVersion = run("plutil", ["-extract", "CFBundleShortVersionString", "raw", infoPlist], { capture: true }).stdout;
  ...
  if (updateSparkle && !existsSync(generatedAppcast)) throw new Error("The preserved macOS result is missing appcast.xml");
  ...
  if (tagCommit !== state.source_sha) throw new Error(`Published tag resolves to ${tagCommit ?? "missing"}; expected ${state.source_sha}`);
  ...
  if (updateSparkle) {
    const remoteMainBefore = run("git", ["ls-remote", "origin", "refs/heads/main"], { capture: true }).stdout.split(/\s+/)[0];
    const localMain = run("git", ["rev-parse", "HEAD"], { capture: true }).stdout;
    if (remoteMainBefore !== localMain) throw new Error(`main moved before Sparkle publication (${localMain} -> ${remoteMainBefore})`);
