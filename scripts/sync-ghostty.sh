...
#   scripts/sync-ghostty.sh --regen          # regenerate patches from the current tree
set -euo pipefail
...
patch_files() {
  case "$1" in
    ...
    *) echo "unknown patch: $1" >&2; return 1 ;;
  esac
}
...
case "${1:-}" in
  --regen) regen ;;
  "") echo "usage: $(basename "$0") <upstream-ref> | --regen" >&2; exit 64 ;;
  *) sync "$1" ;;
esac
