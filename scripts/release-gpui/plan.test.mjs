...
describe("Scenario D — Windows-only fix after a partial failure", () => {
  ...
  const fixedCommit = (() => {
    repo.write("scripts/release-gpui/windows.ps1", "# windows\n# qualify gpui::SharedString\n");
...
describe("Beads pin change", () => {
  ...
  beforeAll(() => {
    ...
    repo.write("scripts/beads-release.mjs", "export const BEADS_VERSION = \"1.2.0\";\n");
  ...
  test("rebuilds both gxserver packages and names the pin file", () => {
    ...
    for (const product of ["gxserver-linux-x64", "gxserver-linux-arm64"]) {
      expect(plan.products[product].action).toBe("build");
      expect(plan.products[product].reason).toMatch(/scripts\/beads-release\.mjs/u);
    }
  ...
  test("rebuilds macOS because it signs the bd payload and embeds both gxservers", () => {
    ...
    expect(plan.products["macos-arm64"].action).toBe("build");
    expect(plan.products["macos-arm64"].reason).toMatch(
      /embedded .*beads|embedded .*gxserver-linux-x64/u,
    );
  });
});
...
describe("Scenario F — unchanged immutable components", () => {
  ...
  test("reuses complete component tags instead of rebuilding them", () => {
    ...
    expect(plan.components.cef.reason).toMatch(
      /already has darwin-arm64, linux-x64, windows-arm64, windows-x64/u,
    );
