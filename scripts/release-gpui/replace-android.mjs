...
if (process.env.GITHUB_SHA && manifest.source_sha !== process.env.GITHUB_SHA) {
  throw new Error(`Android artifact source ${manifest.source_sha ?? "missing"} does not match ${process.env.GITHUB_SHA}`);
}
...
if (
  ...
  manifest.artifacts[0].size !== statSync(apkPath).size
) {
...
const nonAndroidDigests = new Map(
  release.assets
    .filter((asset) => asset.name !== apkName)
...
if (unexpectedAssets.length > 0) {
  throw new Error(`Unexpected assets appeared during Android replacement: ${unexpectedAssets.map((asset) => asset.name).join(", ")}`);
}
